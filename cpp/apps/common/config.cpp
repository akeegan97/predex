...
std::optional<predex::AppConfig> build_app_config(const nlohmann::json& root,
                                                  std::string& error_out,
                                                  const AppConfigParseOptions options) {
    ...
    const auto audit_it = root.find("audit");
    if (audit_it != root.end() && audit_it->is_object()) {
    ...
    if (oms_transport_it != root.end() && oms_transport_it->is_object()) {
        ...
        config.oms_transport.rest_worker_count =
            read_size(oms_transport, "rest_worker_count", config.oms_transport.rest_worker_count);
    ...
    if (options.require_public_channels && config.kalshi_ws.channels.empty()) {
        error_out = "kalshi.channels must contain at least one subscription channel";
        return std::nullopt;
    }
    if (options.require_credentials &&
        (config.kalshi_ws.key_id.empty() || config.kalshi_ws.private_key_pem.empty())) {
...
std::optional<AppConfig> load_app_config(const std::string& config_path,
    ...
    if (!config_stream) {
        error_out = "Failed to open config: " + config_path;
        return std::nullopt;
    }
    ...
    try {
        ...
        error_out = "Failed to parse config JSON: " + std::string(exception.what());
        return std::nullopt;
    }

    return build_app_config(root, error_out, options);
}
