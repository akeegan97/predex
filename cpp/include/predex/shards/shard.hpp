...
inline void log_correction(const predex::core::ingest::kalshi::FrameHandle& handle,
    ...
    std::fprintf(stdout,
                 ...
                 time_buf, event.meta.event_id, event.meta.market_id, handle.sid_,
                 ...
                 static_cast<unsigned long long>(post_book.corrected_negative_level_count));
    std::fflush(stdout);
}

[[gnu::noinline, gnu::cold]]
inline void log_desync(const predex::core::ingest::kalshi::FrameHandle& handle,
                       const PreApplyBookSnapshot& snapshot, const BookState* post_book,
                       const predex::internal::NormalizedEvent& event,
                       std::unordered_set<std::uint64_t>& logged_keys,
                       ShardDesyncReason reason) noexcept {
    ...
    const std::uint64_t desync_key = (static_cast<std::uint64_t>(event.meta.event_id) << 32U) |
    ...
    std::fprintf(stdout,
                 ...
                 time_buf, event.meta.event_id, event.meta.market_id, handle.sid_,
                 static_cast<unsigned long long>(event.effective_sequence_id().value_or(0)),
                 raw_event_type_name(handle.event_type_), parsed_event_type_name(event.type),
                 desync_reason_name(reason), snapshot.has_snapshot ? "true" : "false",
                 snapshot.desynced ? "true" : "false",
                 static_cast<unsigned long long>(snapshot.last_seq_id.value_or(0)), side_name(side),
                 static_cast<long long>(price_ticks), static_cast<long long>(qty_lots),
                 static_cast<long long>(existing_qty_lots),
                 static_cast<long long>(updated_qty_lots), pending_delta_count,
                 static_cast<unsigned long long>(recovery_snapshot_count));
    if (post_book != nullptr && !post_book->recent_updates.empty()) {
        std::fprintf(stdout, "[%s] SHARD | phase=recent_updates | event_id=%u | market_id=%u",
                     time_buf, event.meta.event_id, event.meta.market_id);
        for (const auto& update : post_book->recent_updates) {
            std::fprintf(stdout, " | type=%s,seq=%llu,side=%s,price=%lld,qty=%lld",
                         parsed_event_type_name(update.type),
                         static_cast<unsigned long long>(update.seq_id.value_or(0)),
                         side_name(update.side), static_cast<long long>(update.price_ticks),
...
template <typename Bundle> class Shard {
  public:
...
//NOLINTNEXTLINE 
        ...
        : input_queue_(input_queue), frame_pool_(frame_pool), logger_queue_(logger_queue),
          recycle_queue_(recycle_queue), event_store_(event_store), bundle_(std::move(bundle)) {}
  ...
  private:
    ...
    [[nodiscard]] const ingest::kalshi::KalshiFrame*
    get_frame(const ingest::kalshi::FrameHandle& handle) noexcept {
    ...
    [[nodiscard]] ProcessOneResult apply_event(const ingest::kalshi::FrameHandle& handle,
        ...
        if (post_book != nullptr && post_book->corrected_negative_level_count >
                                        pre_snap.corrected_negative_level_count) [[unlikely]] {
            detail::log_correction(handle, *post_book, event);
        }

        if (apply_result.desync_reason != ShardDesyncReason::kNone) [[unlikely]] {
            detail::log_desync(handle, pre_snap, post_book, event, logged_desync_keys_,
                               apply_result.desync_reason);
        }
