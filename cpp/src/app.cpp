...
void log_public_ws_event(const char* phase, std::uint64_t generation,
    ...
    std::fprintf(stdout, "[%s] WS | scope=public | generation=%llu | phase=%s%s%s\n", time_buf,
                 ...
                 detail == nullptr ? "" : " | detail=", detail == nullptr ? "" : detail);
    std::fflush(stdout);
}
...
struct App::Runtime {
    ...
    using ShardPipeline = predex::core::shards::kalshi::DefaultShardPipeline<
        LocalRiskManager, MonotonicArbStrategy, CdfViolationStrategy, MarketMakingStrategy,
    ...
    static bool build_event_definitions_by_shard(
        const std::vector<MarketRouteConfig>& routes, std::size_t shard_count,
        ...
        for (const auto& route : routes) {
            ...
            const auto duplicate_market = std::find_if(
                accumulator.markets.begin(), accumulator.markets.end(),
    ...
    std::vector<std::jthread>
        shard_threads; // One shard per thread, shard owning strategy & local risk via its pipeline.
...
App::Runtime::Runtime(AppConfig config_in)
    ...
    recycle_from_logger = std::make_unique<FrameQueue>(config.pipeline.frame_pool_capacity);
    recycle_from_router = std::make_unique<FrameQueue>(config.pipeline.frame_pool_capacity);
    ...
    oms_command_queue = std::make_unique<OmsCommandQueue>(config.pipeline.shard_input_capacity);
    ...
    if (!build_event_definitions_by_shard(config.market_routes, config.pipeline.shard_count,
                                          event_definitions_by_shard, event_definition_error)) {
        init_ok = false;
    ...
    for (std::size_t shard_index = 0; shard_index < config.pipeline.shard_count; ++shard_index) {
        if (!event_stores[shard_index].initialize(event_definitions_by_shard[shard_index])) {
            init_ok = false;
            set_error("Failed to initialize event store for shard " + std::to_string(shard_index));
            return;
        }
    }
    ...
    shard_dispatch = std::make_unique<core::routing::kalshi::ShardDispatch>(shard_input_queue_ptrs);

    router = std::make_unique<core::routing::kalshi::Router>(
        *io_to_router_queue, frame_pool, market_registry, *shard_dispatch, *router_to_logger_queue,
        router_audit_queue.get(), *recycle_from_router);

    io_writer = std::make_unique<core::ingest::kalshi::IOWriter>(frame_pool, *io_to_router_queue,
                                                                 recycle_input_queue_ptrs);

    tape_logger = std::make_unique<core::tape::kalshi::Logger>(
        logger_input_queue_ptrs, frame_pool, *recycle_from_logger, config.tape.output_path);
    audit_logger = std::make_unique<AuditLogger>(audit_input_queue_ptrs, config.audit.output_path);
    ...
    {
        using AsyncRestConnection = predex::core::oms::kalshi::gateway::AsyncRestConnection;
        ...
        using SessionPoolConfig = predex::core::oms::kalshi::gateway::SessionPoolConfig;
    ...
    oms = std::make_unique<Oms>(shard_to_oms_intent_queue_ptrs, oms_to_shard_decision_queue_ptrs,
    ...
    for (std::size_t i = 0; i < config.pipeline.shard_count; ++i) {
        shards.push_back(std::make_unique<Shard>(*shard_input_queues[i], frame_pool,
                                                 *shard_to_logger_queues[i],
                                                 *recycle_from_shards[i], event_stores[i],
                                                 ShardPipeline{
...
bool App::Runtime::start() {
    ...
    for (std::size_t i = 0; i < config.pipeline.shard_count; ++i) {
        shard_threads.emplace_back(
            [this, i](const std::stop_token& stop_token) { shard_loop(i, stop_token); });
    }
    oms_thread = std::jthread([this](const std::stop_token& stop_token) { oms_loop(stop_token); });
    if (config.oms_transport.enabled) {
        oms_gateway_thread = std::jthread(
            [this](const std::stop_token& stop_token) { oms_gateway_loop(stop_token); });
    }
    logger_thread =
        std::jthread([this](const std::stop_token& stop_token) { logger_loop(stop_token); });
    audit_thread =
...
void App::Runtime::io_loop(const std::stop_token& stop_token) {
    ...
    while (running.load(std::memory_order_acquire) && !stop_token.stop_requested()) {
        ...
        if (recv_result.status == websocket::RecvStatus::kClosed) {
            ...
            if (!ws_session.connect()) {
                log_public_ws_event("reconnect_failed", next_generation,
        ...
        if (recv_result.status == websocket::RecvStatus::kError) {
            log_public_ws_event("recv_error", public_ws_generation_.load(std::memory_order_acquire),
                                ws_session.last_error().c_str());
            set_error(ws_session.last_error());
...
void App::Runtime::oms_loop(const std::stop_token& stop_token) {
    ...
    while (running.load(std::memory_order_acquire) && !stop_token.stop_requested()) {
        const auto result =
            oms->pump(config.pipeline.shard_input_capacity, config.pipeline.shard_input_capacity);
        if (result.code == predex::core::oms::kalshi::OmsProcessCode::kError) {
...
void App::Runtime::run() {
    ...
    constexpr std::int64_t kHealthDumpIntervalMs =
...
void App::Runtime::print_health_status() const noexcept {
    ...
    const std::uint64_t oms_processed_kalshi_events = oms ? oms->processed_kalshi_event_count() : 0;
    const std::uint64_t oms_emitted_decisions = oms ? oms->emitted_decision_count() : 0;
    const std::uint64_t oms_emitted_transport = oms ? oms->emitted_transport_count() : 0;
    const std::uint64_t oms_emitted_lifecycle = oms ? oms->emitted_lifecycle_count() : 0;
    const std::uint64_t oms_rejected_decisions = oms ? oms->rejected_decision_count() : 0;
    const auto gateway_telem = oms_gateway ? oms_gateway->telemetry()
    ...
    const double gateway_mean_ingress_to_sequence_ms =
        mean_ms(gateway_telem.total_ingress_to_sequence_ns);
    const double gateway_mean_sequence_to_plan_ms =
        mean_ms(gateway_telem.total_sequence_to_plan_ns);
    ...
    const double gateway_mean_wire_to_response_ms =
        mean_ms(gateway_telem.total_wire_to_response_ns);
    ...
    std::fprintf(stdout,
                 ...
                 " | router_frames=%zu | router_drop_bp=%zu | router_shard_bp=%zu | "
                 "router_drop_lifecycle=%zu"
                 " | router_drop_invalid=%zu | router_seq_rejects=%zu | desynced_events=%zu\n",
                 time_buf, halted ? "true" : "false", live_orders,
                 ...
                 gateway_hot_idle, gateway_hot_inflight, gateway_last_latency_ms,
                 gateway_mean_latency_ms, gateway_mean_ingress_to_sequence_ms,
                 gateway_mean_sequence_to_plan_ms, gateway_mean_queue_to_plan_ms,
                 gateway_mean_plan_to_admit_ms, gateway_mean_admit_to_start_ms,
                 gateway_mean_start_to_wire_ms, gateway_mean_wire_to_response_ms,
                 ...
                 telem.processed_frames_, telem.dropped_backpressure_,
                 telem.shard_backpressure_to_logger_, telem.dropped_unknown_ticker_lifecycle_,
                 telem.dropped_invalid_, telem.sequence_rejects_, desynced_events);
    std::fflush(stdout);
}
...
App::~App() = default;
bool App::start() { return runtime_->start(); }
