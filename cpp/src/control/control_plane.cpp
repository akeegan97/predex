...
[[nodiscard]] ShardTelemetrySnapshot
to_control_shard_stats(const shard::ShardStats& stats) noexcept {
    return ShardTelemetrySnapshot{
        ...
        .markets_already_awaiting_recovery = stats.markets_already_awaiting_recovery,
        ...
        .ingress_to_book_apply_latency = stats.ingress_to_book_apply_latency,
    };
}

void account_recovery_observation(RecoveryTelemetrySnapshot& telemetry,
...
void account_recovery_fact(RecoveryTelemetrySnapshot& telemetry,
    ...
    if (result.incident_completed) {
        ...
        const auto duration =
            static_cast<std::uint64_t>(std::max<std::int64_t>(0, result.recovery_duration.count()));
        ++telemetry.recovery_duration_samples;
        telemetry.recovery_duration_total_ns += duration;
        telemetry.recovery_duration_max_ns = std::max(telemetry.recovery_duration_max_ns, duration);
        telemetry.recovery_duration_latency.record(duration);
    }
}

[[nodiscard]] std::vector<operator_admin::UnknownMarketTickerStats>
to_operator_unknown_market_ticker_stats(const std::vector<UnknownMarketTickerStats>& samples) {
...
[[nodiscard]] std::vector<std::vector<shard::KalshiEvent>>
build_shard_events(const UniverseSnapshot& snapshot, std::size_t shard_count) {
...
[[nodiscard]] std::shared_ptr<const OrderRouteUniverse>
build_order_route_universe(const UniverseSnapshot& snapshot) {
...
[[nodiscard]] operator_admin::OperatorCounterStatsSnapshot
build_counter_stats_snapshot(const ProcessState& process_state) {
    std::vector<operator_admin::ShardCounterStats> shard_stats;
    shard_stats.reserve(process_state.shard_component_states.size());

    for (std::uint64_t shard_index = 0; shard_index < process_state.shard_component_states.size();
         ++shard_index) {
        ...
        shard_stats.push_back(operator_admin::ShardCounterStats{
            ...
            .markets_already_awaiting_recovery = telemetry.markets_already_awaiting_recovery,
            ...
            .ingress_to_book_apply_latency = telemetry.ingress_to_book_apply_latency,
        });
    }
    ...
    return operator_admin::OperatorCounterStatsSnapshot{
        .status_snapshot =
            operator_admin::OperatorStatusSnapshot{
        ...
        .io_stats =
            operator_admin::IoCounterStats{
                ...
                .unknown_market_ticker_samples = to_operator_unknown_market_ticker_stats(
                    io_state.telemetry.unknown_market_ticker_samples),
        ...
        .router_stats =
            operator_admin::RouterCounterStats{
                ...
                .subscription_barriers_received =
                    router_state.telemetry.subscription_barriers_received,
                .subscription_barriers_delivered =
                    router_state.telemetry.subscription_barriers_delivered,
                .barriers_deferred = router_state.telemetry.barriers_deferred,
                .subscription_recovery_facts_deferred =
                    router_state.telemetry.subscription_recovery_facts_deferred,
                ...
                .wire_to_router_latency = router_state.telemetry.wire_to_router_latency,
                .router_service_latency = router_state.telemetry.router_service_latency,
            },
        .shard_stats = std::move(shard_stats),
        .logger_stats =
            operator_admin::LoggerCounterStats{
                ...
                .shard_to_logger_latency = logger_state.telemetry.shard_to_logger_latency,
                .ingress_to_logger_write_latency =
                    logger_state.telemetry.ingress_to_logger_write_latency,
            },
        .oms_stats =
            operator_admin::OmsCounterStats{
                ...
                .portfolio_reconciliations_requested =
                    oms_state.telemetry.portfolio_reconciliations_requested,
                .portfolio_reconciliations_completed =
                    oms_state.telemetry.portfolio_reconciliations_completed,
                .portfolio_reconciliations_failed =
                    oms_state.telemetry.portfolio_reconciliations_failed,
                ...
                .strategy_response_backpressure =
                    oms_state.telemetry.strategy_response_backpressure,
        ...
        .private_order_feed_stats =
            operator_admin::PrivateOrderFeedCounterStats{
        ...
        .order_rest_stats =
            operator_admin::OrderRestCounterStats{
...
ControlPlane::ControlPlane(OperatorQueues queues, ControlIoQueues io_queues,
                           RouterQueue router_queue, ControlShardQueues shard_queues,
                           ControlLoggerQueue logger_queue, ControlOmsQueues oms_queues,
    ...
    : queues_(queues), io_queues_(io_queues), router_queue_(router_queue),
      shard_queues_(std::move(shard_queues)), logger_queue_(logger_queue), oms_queues_(oms_queues),
      private_order_feed_queues_(private_order_feed_queues), order_rest_queues_(order_rest_queues),
    ...
    while (queues_.operator_command_queue.try_pop(item)) {
        ...
        case operator_admin::OperatorCommandType::kALLOW_TRADING: {
            ...
            if (process_state_.trading_session_phase != TradingSessionPhase::kTRADING) {
                push_error(item.request_id, "cannot allow trading: synthetic trading session is no "
                                            "longer in trading phase");
                break;
            }
            if (!required_components_ready_for_trading()) {
                push_error(item.request_id,
                           "cannot allow trading: required trading components are not ready");
                break;
            }
        ...
        case operator_admin::OperatorCommandType::kDISABLE_TRADING: {
            ...
            process_state_.oms_component_state.trading_enabled = false;
            if (required_components_.oms &&
                !push_oms_command(ControlToOmsCommand{DisableTrading{}})) {
        ...
        case operator_admin::OperatorCommandType::kCANCEL_ALL_ORDERS: {
            ...
            if (!push_oms_command(ControlToOmsCommand{CancelAllOrders{}})) {
                push_error(item.request_id,
                           "cannot cancel all orders: failed to enqueue OMS command");
                break;
            }
    ...
    std::visit(
        [&](auto&& stat) noexcept {
            ...
            if constexpr (std::is_same_v<T, IoConnected>) {
                ...
                const auto result =
                    recovery_coordinator_.handle(stat, std::chrono::steady_clock::now());
...
void ControlPlane::recompute_process_state() noexcept {
    if (process_state_.shutdown_requested) {
        if ((!required_components_.market_data ||
             process_state_.io_component_state.status == ComponentStatus::kSTOPPED) &&
            (!required_components_.private_order_feed ||
             process_state_.private_order_feed_component_state.status ==
                 ComponentStatus::kSTOPPED) &&
...
bool ControlPlane::required_components_faulted() const {
    ...
    if (required_components_.shards) {
        return std::any_of(process_state_.shard_component_states.begin(),
...
bool ControlPlane::required_components_ready_for_capture() const {
    ...
    if (required_components_.shards) {
        ...
        const bool shards_ready =
            std::all_of(process_state_.shard_component_states.begin(),
                        process_state_.shard_component_states.end(),
...
bool ControlPlane::required_components_ready_for_trading() const {
    ...
    if (required_components_.oms && required_components_.order_rest &&
    ...
    if (required_components_.private_order_feed &&
        (process_state_.private_order_feed_component_state.status != ComponentStatus::kLIVE ||
         process_state_.private_order_feed_component_state.installed_universe_version !=
             target_version ||
         process_state_.private_order_feed_component_state.subscribed_universe_version !=
             target_version)) {
        return false;
    }
...
std::uint64_t ControlPlane::install_universe(UniverseSnapshot snapshot) {
    ...
    auto next_universe = std::make_shared<const UniverseSnapshot>(std::move(snapshot));
    auto next_order_universe = build_order_route_universe(*next_universe);

    ++next_universe_version_;
    const auto supersession = recovery_coordinator_.supersede_before_universe(
        next_universe->version, std::chrono::steady_clock::now());
    process_state_.recovery_telemetry.incidents_superseded += supersession.incidents_superseded;
    process_state_.recovery_telemetry.markets_superseded += supersession.markets_superseded;
...
bool ControlPlane::send_active_order_universe_to_private_order_feed() {
    if (active_order_universe_ == nullptr ||
        private_order_feed_queues_.control_to_private_order_feed_queue == nullptr) {
        return false;
    }
    return private_order_feed_queues_.control_to_private_order_feed_queue->try_push(
        ControlToPrivateOrderFeedCommand{
            ApplyOrderRouteUniverse{.snapshot = active_order_universe_}});
}

bool ControlPlane::send_active_order_universe_to_order_rest() {
    if (active_order_universe_ == nullptr ||
        order_rest_queues_.control_to_order_rest_queue == nullptr) {
        return false;
    }
...
bool ControlPlane::push_private_order_feed_command( // NOLINT
    const ControlToPrivateOrderFeedCommand& cmd) {
...
bool ControlPlane::push_shard_command(std::uint32_t shard_index,
                                      shard::ControlToShardCommand command) {
    ...
    for (std::uint32_t shard_index = 0; shard_index < shard_count; ++shard_index) {
        ...
        if (push_shard_command(shard_index, shard::ControlToShardCommand{command})) {
            ...
            if (shard_index < process_state_.shard_component_states.size()) {
                process_state_.shard_component_states[shard_index].status =
                    ComponentStatus::kQUIESCING;
            }
        } else {
            ++result.commands_pushed_failure;
        }
        ...
        std::visit(
            [&](auto&& m) { // NOLINT
                ...
                if constexpr (std::is_same_v<T, router::RouterTelemetry>) {
                    process_state_.router_component_state.telemetry.total_frames_seen =
                        m.total_frames_seen;
                    process_state_.router_component_state.telemetry.frames_to_shards =
                        m.frames_to_shards;
                    process_state_.router_component_state.telemetry.frames_to_logger =
                        m.frames_to_logger;
                    process_state_.router_component_state.telemetry.frames_recycled =
                        m.frames_recycled;
                    process_state_.router_component_state.telemetry.market_barriers_received =
                        m.market_barriers_received;
                    process_state_.router_component_state.telemetry.market_barriers_delivered =
                        m.market_barriers_delivered;
                    process_state_.router_component_state.telemetry.subscription_barriers_received =
                        m.subscription_barriers_received;
                    process_state_.router_component_state.telemetry
                        .subscription_barriers_delivered = m.subscription_barriers_delivered;
                    process_state_.router_component_state.telemetry.barriers_deferred =
                        m.barriers_deferred;
                    process_state_.router_component_state.telemetry
                        .subscription_recovery_facts_deferred =
                        m.subscription_recovery_facts_deferred;
                    process_state_.router_component_state.telemetry.shard_queue_depth_high_water =
                        m.shard_queue_depth_high_water;
                ...
                if constexpr (std::is_same_v<T, router::OutOfSequenceFrame>) {
                    process_state_.router_component_state.last_error =
                        "Out of sequence frame detected: sid " + std::to_string(m.sid) +
                        " sequence " + std::to_string(m.sequence);
                }
                if constexpr (std::is_same_v<T, router::RouterHandleLeak>) {
                    process_state_.router_component_state.telemetry.leaked_handles++;
                    process_state_.router_component_state.last_error =
                        "Handle leak detected for universe version " +
                        std::to_string(m.universe_version) + " shard index " +
                        std::to_string(m.shard_index);
                    process_state_.router_component_state.status =
                        ComponentStatus::kFAULTED; // major issue, needs attention
                }
                if constexpr (std::is_same_v<T, router::OrderBookSubscriptionBarrierDelivered>) {
                    ...
                    try {
                        const auto observation = recovery_coordinator_.observe(
                            m, *active_universe_, std::chrono::steady_clock::now());
                        account_recovery_observation(process_state_.recovery_telemetry,
                                                     observation);
                    } catch (...) {
                        ++process_state_.recovery_telemetry.observations_rejected;
    ...
    std::visit(
        [&](auto&& stat) {
            ...
            if constexpr (std::is_same_v<T, shard::ShardUniverseInstalled>) {
                ...
                try {
                    const auto result = recovery_coordinator_.observe(
                        stat, *active_universe_, std::chrono::steady_clock::now());
                    account_recovery_observation(process_state_.recovery_telemetry, result);
                } catch (...) {
                    ++process_state_.recovery_telemetry.observations_rejected;
                    recovery_orchestration_faulted_ = true;
                    auto* shard_state = ensure_shard_state(stat.shard_index);
                    shard_state->status = ComponentStatus::kFAULTED;
                    shard_state->last_error = "Failed to create market recovery incident";
                }
            } else if constexpr (std::is_same_v<T, shard::ShardRecoverySnapshotApplied>) {
                if (active_universe_ == nullptr) {
                    return;
                }
                const auto result = recovery_coordinator_.handle(stat, *active_universe_,
    ...
    std::visit(
        [&](auto&& stat) noexcept {
            ...
            if constexpr (std::is_same_v<T, LoggerStarted>) {
                process_state_.logger_component_state.status = ComponentStatus::kLIVE;
                process_state_.logger_component_state.output_file_path =
                    std::move(stat.output_file_path);
    ...
    std::visit(
        [&](auto&& stat) noexcept {
            ...
            if constexpr (std::is_same_v<T, OmsReady>) {
                ...
                process_state_.oms_component_state.installed_universe_version =
                    stat.telemetry.installed_universe_version;
                ...
                process_state_.oms_component_state.telemetry.uncertain_orders =
                    stat.uncertain_orders;
            }
        },
        status);
}
...
// NOLINTNEXTLINE - bugprone-exception-escape std::visit will not hit it's valueless_by_exception
void ControlPlane::apply_private_order_feed_status(
    PrivateOrderFeedToControlStatus&& status) noexcept {
    std::visit(
        [&](auto&& stat) noexcept {
            using T = std::decay_t<decltype(stat)>;
            if constexpr (std::is_same_v<T, PrivateOrderFeedConnected>) {
                ...
                process_state_.private_order_feed_component_state.last_error =
                    std::move(stat.reason);
                ...
                process_state_.private_order_feed_component_state.connected = false;
                process_state_.private_order_feed_component_state.last_error =
                    std::move(stat.error_message);
            } else if constexpr (std::is_same_v<T, PrivateOrderFeedTelemetry>) {
                process_state_.private_order_feed_component_state.telemetry =
                    std::move(stat.telemetry);
            }
        },
        std::move(status));
}
    ...
    std::visit(
        [&](auto&& stat) noexcept {
            ...
            if constexpr (std::is_same_v<T, OrderRestReady>) {
                ...
                process_state_.order_rest_component_state.status = ComponentStatus::kFAULTED;
                process_state_.order_rest_component_state.enabled = false;
                process_state_.order_rest_component_state.last_error =
                    std::move(stat.error_message);
...
RecoveryPumpResult ControlPlane::process_recovery(RecoveryCoordinator::TimePoint now) noexcept {
    ...
    process_state_.recovery_telemetry.request_ack_timeouts += timeouts.request_ack_timeouts;
    process_state_.recovery_telemetry.snapshot_timeouts += timeouts.snapshot_timeouts;
    process_state_.recovery_telemetry.retries_scheduled += timeouts.retries_scheduled;
    process_state_.recovery_telemetry.markets_failed += timeouts.markets_failed;
    process_state_.recovery_telemetry.active_incidents =
        recovery_coordinator_.active_incident_count();
    process_state_.recovery_telemetry.active_markets = recovery_coordinator_.active_market_count();
    if (!process_state_.io_component_state.connected) {
        return result;
    }
    if (process_state_.io_component_state.status != ComponentStatus::kLIVE) {
        return result;
    }
    if (process_state_.io_component_state.installed_universe_version != active_universe_->version) {
        return result;
    }
    if (process_state_.io_component_state.subscribed_universe_version !=
        active_universe_->version) {
        return result;
    }

    const auto commands = recovery_coordinator_.next_pending_commands(now, kMaxCommandsPerPump);
