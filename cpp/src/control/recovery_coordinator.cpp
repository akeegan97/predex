...
[[nodiscard]] bool
valid_targeted_incident_source(const predex::shard::ShardMarketRecoveryRequired& fact) noexcept {
...
RecoveryCoordinator::observe(const shard::ShardMarketRecoveryRequired& fact,
                             const UniverseSnapshot& active_universe, TimePoint now) {
    ...
    if (fact.incident.origin == ingest::kalshi::IntegrityIncidentOrigin::kUNKNOWN ||
    ...
    const auto route_it = std::find_if(
        active_universe.market_routes.begin(), active_universe.market_routes.end(),
        [&fact](const UniverseMarketRoute& route) { return route.market_id == fact.market_id; });
    ...
    RecoveryIncidentState incident{
        ...
        .sid = fact.sid,
        .reason = fact.reason,
        .market_recoveries = {MarketRecoveryState{
...
RecoveryObservationResult RecoveryCoordinator::observe( // NOLINT
    ...
    if (fact.incident.origin != ingest::kalshi::IntegrityIncidentOrigin::kWIRE_SESSION ||
        fact.incident.producer_index != 0 || fact.incident.incident_id == 0) {
        return reject(RecoveryObservationCode::kINVALID_INCIDENT);
    }
    if (fact.reason != ingest::kalshi::BookInvalidationReason::kEXCHANGE_SEQUENCE_GAP) {
        return reject(RecoveryObservationCode::kINVALID_REASON);
    }
    ...
    try {
        ...
        for (const auto& market : incident_it->second.market_recoveries) {
            const auto inserted = active_recovery_by_market_.emplace(market.market_id, recovery_id);
            if (!inserted.second) {
...
RecoverySupersessionSummary
RecoveryCoordinator::supersede_before_universe(std::uint64_t new_universe_version,
    ...
    for (auto& [recovery_id, incident] : incidents_) {
        ...
        for (auto& market : incident.market_recoveries) {
            const auto active_it = active_recovery_by_market_.find(market.market_id);
            if (active_it != active_recovery_by_market_.end() && active_it->second == recovery_id) {
...
RecoveryCoordinator::next_pending_command(TimePoint now) const noexcept {
    ...
    for (const auto& [recovery_id, incident] : incidents_) {
        for (const auto& market : incident.market_recoveries) {
            ...
            if (market.attempts_sent == std::numeric_limits<std::uint32_t>::max()) {
                continue;
            }

            const bool comes_first = selected_incident == nullptr ||
...
std::vector<RecoverMarketIo>
RecoveryCoordinator::next_pending_commands(TimePoint now, std::size_t maximum_commands) const {
    ...
    for (const auto& [recovery_id, incident] : incidents_) {
        const bool has_ready_market = std::any_of(
            incident.market_recoveries.begin(), incident.market_recoveries.end(),
            [now](const MarketRecoveryState& market) {
                return market.phase == MarketRecoveryPhase::kPENDING_REQUEST &&
                       market.next_attempt_time <= now &&
                       market.attempts_sent != std::numeric_limits<std::uint32_t>::max();
            });
        if (has_ready_market &&
            (selected_incident == nullptr || recovery_id < selected_incident->recovery_id)) {
            selected_incident = &incident;
        }
    }
    if (selected_incident == nullptr) {
    ...
    commands.reserve(std::min(maximum_commands, selected_incident->market_recoveries.size()));
...
bool RecoveryCoordinator::mark_command_enqueued(const RecoverMarketIo& command,
                                                TimePoint now) noexcept {
    ...
    const auto market_it =
        std::find_if(incident.market_recoveries.begin(), incident.market_recoveries.end(),
    ...
    const auto active_it = active_recovery_by_market_.find(command.market_id);

    if (active_it == active_recovery_by_market_.end() || active_it->second != command.recovery_id) {
        return false;
    }

    if (market_it->phase != MarketRecoveryPhase::kPENDING_REQUEST ||
        market_it->next_attempt_time > now ||
        market_it->attempts_sent == std::numeric_limits<std::uint32_t>::max() ||
...
RecoveryFactResult RecoveryCoordinator::handle(const IoRecoveryRequestAccepted& fact,
                                               TimePoint now) noexcept {
    RecoveryFactResult result{.recovery_id = fact.recovery_id, .market_id = fact.market_id};
    ...
    const auto market_iter = std::find_if(
        incident.market_recoveries.begin(), incident.market_recoveries.end(),
        [&fact](const MarketRecoveryState& market) { return market.market_id == fact.market_id; });
    ...
    if (active_iter == active_recovery_by_market_.end() ||
        active_iter->second != fact.recovery_id) {
        result.disposition = RecoveryFactDisposition::kIGNORED;
        return result;
    }
...
RecoveryFactResult RecoveryCoordinator::handle(const IoRecoveryRequestFailed& fact,
                                               TimePoint now) noexcept {
    ...
    if (active_it == active_recovery_by_market_.end() || active_it->second != fact.recovery_id) {
        result.disposition = RecoveryFactDisposition::kIGNORED;
        return result;
    }
    ...
    market.next_attempt_time = now + retry_backoff(market.attempts_sent);

    result.disposition = RecoveryFactDisposition::kAPPLIED;
    result.effect = RecoveryFactEffect::kRETRY_SCHEDULED;
    return result;
}
RecoveryFactResult RecoveryCoordinator::handle(const shard::ShardRecoverySnapshotApplied& fact,
    ...
    if (active_universe.version == 0 || fact.universe_version != active_universe.version) {
        result.disposition = RecoveryFactDisposition::kIGNORED;
        return result;
    }
    ...
    if (route_it == active_universe.market_routes.end() ||
        route_it->shard_index != fact.shard_index || route_it->shard_index != market.shard_index ||
    ...
    if (market.phase == MarketRecoveryPhase::kPENDING_REQUEST && market.attempts_sent == 0) {
        return result;
    }
    ...
    result.incident_completed =
        std::all_of(incident.market_recoveries.begin(), incident.market_recoveries.end(),
                    [](const MarketRecoveryState& candidate) {
                        return candidate.phase == MarketRecoveryPhase::kSNAPSHOT_APPLIED ||
                               candidate.phase == MarketRecoveryPhase::kSUPERSEDED;
                    });

    if (result.incident_completed) {
        result.recovery_duration =
            std::chrono::duration_cast<std::chrono::nanoseconds>(now - incident.started_at);
    }

    return result;
}

RecoveryTimeoutSummary RecoveryCoordinator::expire_timeouts(TimePoint now) noexcept {
...
std::size_t RecoveryCoordinator::active_incident_count() const noexcept {
    ...
    for (const auto& [recovery_id, incident] : incidents_) {
        const bool active =
            std::any_of(incident.market_recoveries.begin(), incident.market_recoveries.end(),
                        [this, recovery_id](const MarketRecoveryState& market) {
                            const auto iterator = active_recovery_by_market_.find(market.market_id);
