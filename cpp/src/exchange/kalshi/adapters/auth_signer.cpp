...
std::string detail::make_rest_signing_payload(std::string_view timestamp_ms,
    ...
    const std::string_view signing_path = request_target.substr(0, query_offset);

    std::string payload;
    payload.reserve(timestamp_ms.size() + args.method.size() + signing_path.size());
...
AuthHeaders AuthSigner::make_rest_auth_headers(const RestAuthArguments& args) const {
    const auto timestamp_ms =
        std::to_string(std::chrono::duration_cast<std::chrono::milliseconds>(
                           ...
                           .count());
    const auto payload = detail::make_rest_signing_payload(timestamp_ms, args);
    return AuthHeaders{
...
std::string AuthSigner::sign_rsa_pss_sha256_base64(std::string_view payload) const {
    ...
    if (EVP_DigestSignInit(md_ctx_guard.get(), &pkey_ctx, EVP_sha256(), nullptr, pkey_.get()) <=
        0) {
        throw std::runtime_error(openssl_error_message("EVP_DigestSignInit failed"));
    }
    if (pkey_ctx == nullptr) {
        throw std::runtime_error(
            openssl_error_message("EVP_DigestSignInit returned null EVP_PKEY_CTX"));
    }
