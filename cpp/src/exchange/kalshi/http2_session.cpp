...
http_method_to_string(predex::exchange::kalshi::HttpMethod method) noexcept {
...
Http2Session::Http2Session(AuthSigner signer, Http2SessionConfig config)
    : signer_(std::move(signer)), config_(std::move(config)) {}

Http2Session::Http2Session(Http2Session&& other) noexcept
    : signer_(std::move(other.signer_)), config_(std::move(other.config_)),
      curl_multi_handle_(other.curl_multi_handle_),
      inflight_requests_(std::move(other.inflight_requests_)),
      last_error_(std::move(other.last_error_)), is_closed_(other.is_closed_),
...
Http2Session::~Http2Session() { close(); }
...
std::string_view Http2Session::last_error() const noexcept { return last_error_; }

bool Http2Session::has_inflight_requests() const noexcept { return !inflight_requests_.empty(); }
...
HttpPollResult Http2Session::poll() {
    ...
    if (multi_code != CURLM_OK) {
        ...
        return HttpPollResult{has_inflight_requests() ? HttpRequestStatus::kIN_FLIGHT
    ...
    while (CURLMsg* msg = curl_multi_info_read(curl_multi_handle_, &queued)) {
        if (msg->msg == CURLMSG_DONE) {
            ...
            auto request_id =
                static_cast<HttpRequestId>(reinterpret_cast<std::uintptr_t>(request_id_pointer));
            ...
            if (msg->data.result != CURLE_OK) {
                ...
                response.error_message =
                    curl_error[0] != '\0'
                        ...
                        : "CURL error: " + std::string{curl_easy_strerror(msg->data.result)};
            }
            response.ok =
                (msg->data.result == CURLE_OK && http_code >= 200 && http_code < 300); // NOLINT
            response.trace = active_req.request.trace;
            response.trace.response_recv_ts_ns = now_ns();
            response.trace.negotiated_protocol = (http_version == CURL_HTTP_VERSION_2_0)
                                                     ? HttpProtocol::kHTTP_2
                                                     : HttpProtocol::kHTTP_1_1;
...
HttpStartResult Http2Session::start_request(HttpRequest request) { // NOLINT
    ...
    active.deadline_ns =
        ...
        static_cast<std::uint64_t>(
            std::chrono::duration_cast<std::chrono::nanoseconds>(config_.request_timeout).count());
    ...
    if (active.request.authenticate) {
        AuthHeaders auth_headers = signer_.make_rest_auth_headers(
            RestAuthArguments{.method = active.method, .path = active.request.target});
    ...
    curl_easy_setopt(active_req_stored.curl_handle, CURLOPT_HTTPHEADER,
                     active_req_stored.curl_headers);
    curl_easy_setopt(active_req_stored.curl_handle, CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_2TLS);
    curl_easy_setopt(active_req_stored.curl_handle, CURLOPT_PRIVATE,
                     reinterpret_cast<void*>(static_cast<std::uintptr_t>(request_id))); // NOLINT
    ...
    curl_easy_setopt(active_req_stored.curl_handle, CURLOPT_ERRORBUFFER,
                     active_req_stored.error_buffer.data());
    curl_easy_setopt(active_req_stored.curl_handle, CURLOPT_TIMEOUT_MS,
                     static_cast<long>(config_.request_timeout.count()));
    curl_easy_setopt(active_req_stored.curl_handle, CURLOPT_CONNECTTIMEOUT_MS,
    ...
    case HttpMethod::kPOST:
        ...
        curl_easy_setopt(active_req_stored.curl_handle, CURLOPT_POSTFIELDS,
                         active_req_stored.body.c_str());
        curl_easy_setopt(active_req_stored.curl_handle, CURLOPT_POSTFIELDSIZE,
                         active_req_stored.body.size());
        break;
    case HttpMethod::kDELETE:
    ...
    const CURLMcode add_result =
        curl_multi_add_handle(curl_multi_handle_, active_req_stored.curl_handle);
    if (add_result != CURLM_OK) {
        last_error_ = "Failed to add CURL easy handle to multi handle: " +
