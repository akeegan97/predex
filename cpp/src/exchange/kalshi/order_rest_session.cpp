...
void OrderRestSession::apply_order_route_universe(
    const std::shared_ptr<const core::control::OrderRouteUniverse>& snapshot) {
    if (!snapshot) {
    ...
    if (!try_push_control_status(
            core::control::OrderRestUniverseApplied{.version = installed_universe_version_})) {
...
void OrderRestSession::drain_oms_commands() {
    ...
    while (!egress_closed_ && inflight_requests_.size() < kMAX_INFLIGHT_REQUESTS &&
           oms_queues_.oms_to_order_rest_queue.try_pop(command)) {
...
void OrderRestSession::enable() noexcept {
    ...
    if (status_.faulted && (!inflight_requests_.empty() || pending_oms_count_ > 0)) {
        status_.last_error =
    ...
    if (!http_session_.warm_up()) {
        fault("failed to warm HTTP/2 session: " + std::string{http_session_.last_error()});
        return;
    }
...
bool OrderRestSession::try_push_control_status(
    core::control::OrderRestToControlStatus status) noexcept {
...
void OrderRestSession::try_send_pending_control_notifications() noexcept {
    if (pending_fault_notification_) {
        if (try_push_control_status(
                control::OrderRestFaulted{.error_message = status_.last_error})) {
            pending_fault_notification_ = false;
        }
        return;
    }
...
void OrderRestSession::handle_control_command(
    const core::control::ControlToOrderRestCommand& command) {
    std::visit(
        [this](const auto& cmd) {
...
bool OrderRestSession::emit_local_reject(const PreparedOrderRestRequest& prepared,
                                         std::string reason) {

    const std::uint64_t reject_ts_ns = now_ns();

    if (const auto* batch = std::get_if<oms::SubmitOrderBatchCmd>(&prepared.source_command)) {
        ...
        const std::size_t safe_order_count =
            std::min<std::size_t>(batch->order_count, batch->orders.size());
        ...
        for (std::size_t i = 0; i < safe_order_count; ++i) {
            ...
            response.order_responses[i] = oms::RestOrderResponse{
                .context =
                    oms::OmsContext{
                        .oms_request_id = leg.oms_request_id,
                        .context = leg.new_order_intent.context,
                    },
        ...
        return send_or_defer_oms_event(oms::KalshiToOmsEvent{std::move(response)});
    }
    ...
    std::visit(
        [&response](const auto& cmd) {
            ...
            if constexpr (std::is_same_v<T, oms::RequestPortfolioReconciliation>) {
                response.context.oms_request_id = cmd.reconciliation_id;
            } else {
...
void OrderRestSession::handle_oms_command(const oms::OmsToKalshiCommand& command) {
    ...
    if (const auto* reconcile_cmd = std::get_if<oms::RequestPortfolioReconciliation>(&command)) {
    ...
    case HttpStartResult::kERROR:
        (void)emit_local_reject(prepared,
                                "HTTP session error: " + std::string{http_session_.last_error()});
        return;
    }
...
bool OrderRestSession::start_portfolio_request(PreparedPortfolioRestRequest prepared) noexcept {
    if (!prepared.ok) {
        fail_portfolio_reconciliation(prepared.error_message);
        return false;
    }
    if (inflight_requests_.size() >= kMAX_INFLIGHT_REQUESTS) {
        fail_portfolio_reconciliation("HTTP session at capacity during portfolio reconciliation");
        return false;
    }
    ...
    if (start_result != HttpStartResult::kACCEPTED) {
        fail_portfolio_reconciliation("failed to start portfolio reconciliation HTTP request");
        return false;
    }
    inflight_requests_.emplace(request_id, InflightRequest{std::move(prepared)});
    ++telemetry_.requests_sent;
...
void OrderRestSession::handle_portfolio_reconciliation(
    ...
    if (command.reconciliation_id == 0) {
        oms::VenuePortfolioSnapshot failed{
            ...
            .result_code = oms::PortfolioReconciliationResultCode::kFAILED,
            .request_ts_ns = command.submission_ts_ns,
            .received_ts_ns = now_ns(),
            .error_message = "portfolio reconciliation id is zero",
        };
        (void)send_or_defer_oms_event(oms::KalshiToOmsEvent{std::move(failed)});
        return;
    }
    if (!status_.enabled || status_.faulted) {
        oms::VenuePortfolioSnapshot failed{
            ...
            .request_ts_ns = command.submission_ts_ns,
            .received_ts_ns = now_ns(),
            .error_message = "portfolio reconciliation requested while order REST is unavailable",
        };
        (void)send_or_defer_oms_event(oms::KalshiToOmsEvent{std::move(failed)});
        return;
    }
    if (command.universe_version == 0 || command.universe_version != installed_universe_version_) {
        oms::VenuePortfolioSnapshot failed{
            .reconciliation_id = command.reconciliation_id,
            .universe_version = command.universe_version,
            .result_code = oms::PortfolioReconciliationResultCode::kFAILED,
            .request_ts_ns = command.submission_ts_ns,
            .received_ts_ns = now_ns(),
            .error_message = "portfolio reconciliation universe does not match order REST universe",
        };
        (void)send_or_defer_oms_event(oms::KalshiToOmsEvent{std::move(failed)});
        return;
    }
    ...
    active_portfolio_reconciliation_.emplace(ActivePortfolioReconciliation{
        .command = command,
    });

    auto prepared = order_rest_adapter_.prepare_portfolio_balance_request(
        command, next_portfolio_http_request_id());
    (void)start_portfolio_request(std::move(prepared));
}

void OrderRestSession::fail_portfolio_reconciliation(std::string reason) noexcept {
    if (!active_portfolio_reconciliation_.has_value()) {
        return;
    }
    telemetry_.last_portfolio_reconciliation_error = reason;
    const auto command = active_portfolio_reconciliation_->command;
    ...
    if (!send_or_defer_oms_event(oms::KalshiToOmsEvent{std::move(failed)})) {
        fault("unable to deliver failed portfolio reconciliation to OMS");
    }
}

void OrderRestSession::complete_portfolio_request(const PreparedPortfolioRestRequest& prepared,
                                                  const HttpResponse& response) noexcept {
    if (!active_portfolio_reconciliation_.has_value() ||
        active_portfolio_reconciliation_->command.reconciliation_id != prepared.reconciliation_id) {
        fault("received stale or unknown portfolio reconciliation response");
        return;
    }

    if (prepared.kind == PortfolioRestRequestKind::kBALANCE) {
        auto completed = order_rest_adapter_.complete_portfolio_balance_request(prepared, response);
        if (!completed.ok) {
            fail_portfolio_reconciliation(std::move(completed.error_message));
            return;
        }

        auto& active = *active_portfolio_reconciliation_;
        active.available_balance_ticks = completed.available_balance_ticks;
        active.portfolio_value_ticks = completed.portfolio_value_ticks;
        active.balance_complete = true;

        auto positions_request = order_rest_adapter_.prepare_portfolio_positions_request(
            active.command, next_portfolio_http_request_id());
        (void)start_portfolio_request(std::move(positions_request));
        return;
    }

    auto completed = order_rest_adapter_.complete_portfolio_positions_request(prepared, response);
    if (!completed.ok) {
        fail_portfolio_reconciliation(std::move(completed.error_message));
        return;
    }
    ...
    auto& active = *active_portfolio_reconciliation_;
    active.positions.insert(active.positions.end(),
    ...
    if (!completed.next_cursor.empty()) {
        auto next_page = order_rest_adapter_.prepare_portfolio_positions_request(
            active.command, next_portfolio_http_request_id(), completed.next_cursor);
...
void OrderRestSession::maybe_finish_portfolio_reconciliation() noexcept {
    ...
    auto active = std::move(*active_portfolio_reconciliation_);
    active_portfolio_reconciliation_.reset();
    ...
    oms::VenuePortfolioSnapshot snapshot{
        ...
        .result_code = oms::PortfolioReconciliationResultCode::kCOMPLETE,
        .available_balance_ticks = active.available_balance_ticks,
        .portfolio_value_ticks = active.portfolio_value_ticks,
        .market_positions = std::move(active.positions),
        .request_ts_ns = active.command.submission_ts_ns,
        .received_ts_ns = now_ns(),
    };
    if (!send_or_defer_oms_event(oms::KalshiToOmsEvent{std::move(snapshot)})) {
...
bool OrderRestSession::defer_oms_event(oms::KalshiToOmsEvent event) noexcept {
    if (pending_oms_count_ >= pending_oms_events_.size()) {
        // component faulted eventually surface to CP to ochestrate recovery/halt strategy from
        // emitting more intents etc.
        return false;
    }
...
void OrderRestSession::drain_pending_oms_events() noexcept {
    while (pending_oms_count_ > 0) {
        ...
        pending_oms_events_[pending_oms_head_ % pending_oms_events_.size()] =
            oms::KalshiToOmsEvent{}; // clear the slot
...
void OrderRestSession::receive_http(std::size_t max_batch_size) noexcept {
    ...
    for (std::size_t i = 0; i < max_batch_size; ++i) {
        ...
        if (poll_result.status == HttpRequestStatus::kIDLE ||
            poll_result.status == HttpRequestStatus::kIN_FLIGHT) {
            break;
        }
        ...
        auto prepared = std::move(inflight_iter->second.prepared);

        inflight_requests_.erase(inflight_iter);

        ++telemetry_.responses_received;

        if (auto* portfolio_prepared = std::get_if<PreparedPortfolioRestRequest>(&prepared)) {
            complete_portfolio_request(*portfolio_prepared, response);
            continue;
        }

        auto* order_prepared = std::get_if<PreparedOrderRestRequest>(&prepared);
        if (order_prepared == nullptr) {
            fault("Received HTTP response for unknown prepared request type");
            break;
        }

        if (order_prepared->command_kind == oms::RestCommandKind::kSUBMIT_ORDER_BATCH) {

            CompletedOrderRestBatchRequest completed =
                order_rest_adapter_.complete_batch_request(*order_prepared, response);
            ...
            if (!send_or_defer_oms_event(oms::KalshiToOmsEvent{std::move(completed.response)})) {
                fault("pending OMS event queue full, unable to send batch response to OMS");
                break;
            }
        ...
        CompletedOrderRestRequest completed =
            order_rest_adapter_.complete_request(*order_prepared, response);
...
void OrderRestSession::maybe_send_telemetry() noexcept {
    ...
    core::control::OrderRestTelemetrySnapshot snapshot{
        ...
        .last_portfolio_reconciliation_error = telemetry_.last_portfolio_reconciliation_error,
    };
...
void OrderRestSession::finish_egress_drain() noexcept {
    if (!egress_closed_ || drain_marker_sent_ || !inflight_requests_.empty() ||
        pending_oms_count_ > 0) {
        return;
    }
    if (!send_or_defer_oms_event(oms::KalshiToOmsEvent{oms::OrderRestEgressDrained{
            .shutdown_epoch = shutdown_epoch_, .completion_ts_ns = now_ns()}})) {
