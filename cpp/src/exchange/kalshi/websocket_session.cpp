...
std::optional<predex::exchange::kalshi::EndpointParts> parse_ws_endpoint(std::string_view endpoint,
    ...
    const std::string authority{path_pos == std::string_view::npos
                                    ? endpoint.substr(host_pos)
                                    : endpoint.substr(host_pos, path_pos - host_pos)};
...
WebSocketSession::WebSocketSession(const IWsAdapter& adapter)
    : ssl_context_(boost::asio::ssl::context::tlsv12_client),
      ws_stream_(std::make_unique<WsStream>(io_context_, ssl_context_)), adapter_(adapter) {
...
bool WebSocketSession::connect() {
    ...
    const auto tls_handshake_result =
        ws_stream_->next_layer().handshake(boost::asio::ssl::stream_base::client, error_code);
...
std::string_view WebSocketSession::last_error() const noexcept { return last_error_; }
...
ReadResult WebSocketSession::recv_text() {
    ...
    return ReadResult{.status = ReadStatus::kMESSAGE,
...
void WebSocketSession::arm_read() {
    ...
    ws_stream_->async_read(
        read_buffer_, [this](boost::beast::error_code error_code, std::size_t bytes_transferred) {
            assert(read_state_ == ReadState::kPENDING);
...
void WebSocketSession::start_active_write() {
    ...
    ws_stream_->async_write(
        boost::asio::buffer(*active_write_), // NOLINT
...
void WebSocketSession::transition_fault(std::string_view operation,
