...
predex::ingest::kalshi::FrameKind frame_kind_from_type(std::string_view type) noexcept {
    ...
    if (type == "market_lifecycle" || type == "market_lifecycle_v2" || type == "event_fee_update") {
...
void KalshiWireSession::run(const std::stop_token& stop_token) {
    while (!stop_token.stop_requested()) {
        ...
        const bool may_consume_message = recovery_statuses_flushed && integrity_barrier_flushed;
...
SequenceObservation KalshiWireSession::observe_sequence(std::uint32_t sid,
                                                        std::uint64_t sequence) noexcept {
    return sequence_observer_.observe(sid, sequence);
}

core::control::MarketDataChannelTelemetrySnapshot*
KalshiWireSession::channel_telemetry(exchange::kalshi::KalshiMarketDataChannel channel) noexcept {
...
void KalshiWireSession::update_capacity_high_water() noexcept {
    telemetry_.frame_pool_in_use_high_water = std::max<std::uint64_t>(
        telemetry_.frame_pool_in_use_high_water, frame_pool_.capacity() - frame_pool_.available());
    telemetry_.router_queue_depth_high_water = std::max<std::uint64_t>(
        telemetry_.router_queue_depth_high_water, router_queue_.producer_size());
}
...
void KalshiWireSession::stamp_handle(FrameHandle& handle, const MarketDataEnvelope& envelope,
...
bool KalshiWireSession::send_or_defer_integrity_barrier(MarketDataPathMessage barrier) noexcept {
    if (pending_integrity_barrier_.has_value()) {
        return false;
    }
...
void KalshiWireSession::send_or_defer_recovery_status(core::control::IoToControlStatus status) {
    if (std::holds_alternative<core::control::IoRecoveryRequestAccepted>(status)) {
        ++telemetry_.snapshot_requests_accepted;
    } else if (std::holds_alternative<core::control::IoRecoveryRequestFailed>(status)) {
        ++telemetry_.snapshot_requests_failed;
    }

    if (pending_recovery_statuses_.empty() &&
        control_queues_.io_to_control_status_queue.try_push(std::move(status))) {
        return;
    }
    // NOLINTNEXTLINE(bugprone-use-after-move) -- try_push consumes its argument only on success;
...
void KalshiWireSession::maybe_send_telemetry() noexcept {
    ...
    if (now < next_telemetry_send_ || !pending_recovery_statuses_.empty()) {
        return;
    }
...
void KalshiWireSession::record_unknown_market_ticker(
    const core::control::UnknownMarketTickerStats& unknown_market_ticker) {
    auto& samples = telemetry_.unknown_market_ticker_samples;
    const auto already_sampled =
...
void KalshiWireSession::drain_control_commands() {
    ...
    while (pending_recovery_statuses_.empty() && pop_control_command(cmd)) {
...
void KalshiWireSession::recover_market(const core::control::RecoverMarketIo& command) {
    ...
    if (desired_universe_ == nullptr || desired_universe_->version != command.universe_version) {
    ...
    if (existing_tag != pending_recovery_by_market_.end()) {
        ...
        const bool same_request = tag.recovery_id == command.recovery_id &&
        ...
        const bool command_still_pending =
            std::any_of(pending_ws_commands_.begin(), pending_ws_commands_.end(),
        ...
        if (!command_still_pending) {
            send_or_defer_recovery_status(core::control::IoRecoveryRequestAccepted{
                .recovery_id = command.recovery_id,
                .universe_version = command.universe_version,
                .market_id = command.market_id,
                .request_attempt = command.request_attempt,
    ...
    pending_recovery_by_market_.emplace(command.market_id,
                                        PendingRecoveryTag{
                                            .recovery_id = command.recovery_id,
                                            .universe_version = command.universe_version,
                                            .request_attempt = command.request_attempt,
    ...
    const auto send_status = ws_session_.send_text(market_data_handler_.build_update_message(
        ws_command_id, static_cast<std::int64_t>(sid), tickers, "get_snapshot"));
    ...
    const auto tag_iterator = pending_recovery_by_market_.find(command.market_id);
    ...
    fail_request("Failed to send recovery snapshot request: " +
                 std::string{ws_session_.last_error()});
}

void KalshiWireSession::apply_universe_snapshot(
    const std::shared_ptr<const core::control::UniverseSnapshot>& snapshot) {
    ...
    if (desired_universe_ != nullptr && desired_universe_->version != snapshot->version) {
    ...
    (void)push_control_status(
        core::control::IoUniverseSnapshotApplied{.version = desired_universe_->version});
...
bool KalshiWireSession::subscribe_active_universe() {
    ...
    for (const auto& channel : desired_channels_) {
        ...
        if (!subscribe_channel(channel, tickers)) {
            ...
            report_fault("Failed to subscribe to channel " +
...
bool KalshiWireSession::subscribe_channel(exchange::kalshi::KalshiMarketDataChannel channel,
...
bool KalshiWireSession::add_markets(exchange::kalshi::KalshiMarketDataChannel channel,
                                    std::span<const std::string> tickers) {
    auto sub_it = active_subscriptions_.find(channel);

    if (sub_it == active_subscriptions_.end()) {
        report_fault("Cannot add markets to channel - subscription not found: " +
                     std::to_string(static_cast<std::uint8_t>(channel)));
        return false;
    }
    if (!sub_it->second.sid.has_value()) {
        report_fault("Cannot add markets to channel - subscription not active: " +
                     std::to_string(static_cast<std::uint8_t>(channel)));
        return false;
    }
...
bool KalshiWireSession::delete_markets(exchange::kalshi::KalshiMarketDataChannel channel,
                                       std::span<const std::string> tickers) {
    auto sub_it = active_subscriptions_.find(channel);

    if (sub_it == active_subscriptions_.end()) {
        report_fault("Cannot delete markets from channel - subscription not found: " +
                     std::to_string(static_cast<std::uint8_t>(channel)));
        return false;
    }

    if (!sub_it->second.sid.has_value()) {
        report_fault("Cannot delete markets from channel - subscription not active: " +
                     std::to_string(static_cast<std::uint8_t>(channel)));
        return false;
    }
    auto* subscription = &sub_it->second;
...
void KalshiWireSession::handle_ws_control_response(std::span<const std::byte> payload) {
    ...
    if (!read_string(root, "type", response_type)) {
        if (pending.recovery_context.has_value()) {
            ...
            const auto tag_iterator = pending_recovery_by_market_.find(context.market_id);
            if (tag_iterator != pending_recovery_by_market_.end() &&
    ...
    const auto subscription_iterator = active_subscriptions_.find(pending.channel);
    if (subscription_iterator == active_subscriptions_.end()) {
    ...
    case WsCommandKind::kSUBSCRIBE: {
        ...
        if (sid < 0 ||
            static_cast<std::uint64_t>(sid) > std::numeric_limits<std::uint32_t>::max()) {
            subscription.phase = SubscriptionPhase::kFAULTED;
        ...
        if (sequence_observer_.active(normalized_sid) &&
            ...
            subscription.phase = SubscriptionPhase::kFAULTED;
            subscription.last_error = "Subscribed response reused an active sid";
            report_fault(subscription.last_error);
            return;
        }

        if (subscription.sid.has_value() && *subscription.sid != normalized_sid) {
            sequence_observer_.deactivate(*subscription.sid);
        }
...
void KalshiWireSession::publish_market_data_frame(std::span<const std::byte> payload,
    ...
    simdjson::padded_string padded_payload{std::string_view{payload_data, payload.size()}};
    MarketDataEnvelope envelope{};
    const EnvelopeParseCode parse_result = parse_market_data_envelope(padded_payload, envelope);

    const bool sequence_available = parse_result == EnvelopeParseCode::kOK ||
    ...
    if (sequence_available) {
        sequence_observation = observe_sequence(envelope.sid, envelope.sequence);
        if (sequence_observation.code == SequenceObservationCode::kINACTIVE_SID) {
            ++telemetry_.frames_dropped;
        ...
        auto* const channel_stats = channel_telemetry(sequence_observation.channel);
        ...
        if (sequence_observation.code == SequenceObservationCode::kGAP &&
            ...
            const OrderBookSubscriptionInvalidationBarrier barrier{
                .universe_version = desired_universe_ != nullptr ? desired_universe_->version : 0,
                .incident =
                    {
                ...
                .sid = envelope.sid,
                .expected_sequence = sequence_observation.expected_sequence,
                .observed_sequence = sequence_observation.observed_sequence,
                .reason = BookInvalidationReason::kEXCHANGE_SEQUENCE_GAP,
            };
            if (!send_or_defer_integrity_barrier(MarketDataPathMessage{barrier})) {
                ++telemetry_.frames_dropped;
                return;
            }
        }
    ...
    const StampHandleCode route_result = resolve_market_route(envelope, route);
    ...
    case StampHandleCode::kUNKNOWN_MARKET_TICKER:
        ...
        if (sequence_available) {
            if (auto* stats = channel_telemetry(sequence_observation.channel); stats != nullptr) {
    ...
    const bool affects_order_book = envelope.kind == FrameKind::kORDERBOOK_SNAPSHOT ||
                                    envelope.kind == FrameKind::kORDERBOOK_DELTA;
    const auto emit_market_barrier = [this, &envelope, route](BookInvalidationReason reason) {
        ...
        return send_or_defer_integrity_barrier(MarketDataPathMessage{barrier});
    };
    const auto fail_correlated_recovery = [this, &envelope, route](std::string reason) {
        if (envelope.kind != FrameKind::kORDERBOOK_SNAPSHOT) {
            return;
        }
        const auto tag_iterator = pending_recovery_by_market_.find(route->market_id);
        if (tag_iterator == pending_recovery_by_market_.end()) {
            return;
        }
        ...
        for (auto iterator = pending_ws_commands_.begin();
            ...
            if (context.has_value() && context->recovery_id == tag.recovery_id &&
    ...
    if (!frame_pool_.try_acquire(handle)) {
        ...
        if (!frame_pool_.try_acquire(handle)) {
            ...
            if (affects_order_book) {
                (void)emit_market_barrier(BookInvalidationReason::kWIRE_POOL_EXHAUSTION);
    ...
    if (latency_channel_index < core::control::kMarketDataChannelCount) {
        predex::utils::record_elapsed_ns(telemetry_.wire_service_latency[latency_channel_index],
                                         handle.ingress_ts_ns, handle.wire_publish_ts_ns);
    }
    ...
    if (!router_queue_.try_push(MarketDataPathMessage{handle})) {
        ...
        if (!logger_queue_.try_push(handle)) {
            ...
            if (sequence_available) {
                if (auto* stats = channel_telemetry(sequence_observation.channel);
                    stats != nullptr) {
        ...
        if (affects_order_book) {
            (void)emit_market_barrier(BookInvalidationReason::kWIRE_TO_ROUTER_DELIVERY_LOSS);
            fail_correlated_recovery("Recovery snapshot could not be delivered to the router");
        }
        return;
    }
    update_capacity_high_water();
    ...
    if (recovery_tag.has_value()) {
        ...
        if (tag_iterator != pending_recovery_by_market_.end() &&
            ...
            tag_iterator->second.universe_version == recovery_tag->universe_version &&
            tag_iterator->second.request_attempt == recovery_tag->request_attempt) {
            pending_recovery_by_market_.erase(tag_iterator);
        }
    }
    ++telemetry_.frames_published;
...
IncomingMessageKind
KalshiWireSession::classify_incoming_message(std::span<const std::byte> payload) {
    if (payload.empty()) {
    ...
    if (type == "orderbook_delta" || type == "orderbook_snapshot" || type == "trade" ||
        type == "market_lifecycle" || type == "market_lifecycle_v2" || type == "event_fee_update") {
        return IncomingMessageKind::kMARKET_DATA;
    }

    if (type == "ok" || type == "error" || type == "subscribed" || type == "unsubscribed") {
        return IncomingMessageKind::kIGNORE;
    }

    std::uint64_t sid{};
    std::uint64_t sequence{};
    if (read_uint64(root, "sid", sid) && read_uint64(root, "seq", sequence)) {
        return IncomingMessageKind::kMARKET_DATA;
    }
...
EnvelopeParseCode
KalshiWireSession::parse_market_data_envelope(simdjson::padded_string_view payload,
    ...
    if (doc_result.error() != simdjson::SUCCESS) {
        return EnvelopeParseCode::kINVALID_JSON;
    }
    auto root_result = doc_result.get_object();
    if (root_result.error() != simdjson::SUCCESS) {
        return EnvelopeParseCode::kINVALID_JSON;
...
bool KalshiWireSession::unsubscribe_channel(exchange::kalshi::KalshiMarketDataChannel channel) {
    ...
    if (sub_it == active_subscriptions_.end()) {
        report_fault("Cannot unsubscribe from channel - not currently subscribed: " +
                     std::to_string(static_cast<std::uint8_t>(channel)));
        return false;
    }
    if (!sub_it->second.sid.has_value()) {
        report_fault("Cannot unsubscribe from channel - subscription has no SID: " +
                     std::to_string(static_cast<std::uint8_t>(channel)));
        return false;
    }
    ...
    if (subscription.phase != SubscriptionPhase::kSUBSCRIBED) {
        report_fault("Cannot unsubscribe from channel - subscription is not active: " +
                     std::to_string(static_cast<std::uint8_t>(channel)));
        return false;
    }

    const std::int64_t sid =
        *subscription.sid; // NOLINT bugprone-unchecked-optional-access -> it is checked above
    ...
    // NOTE: might change build_unsubscribe_message to just take a single sid instead of a span
    // since it's only ever called one channel at a time here.
    const auto send_status = ws_session_.send_text(
