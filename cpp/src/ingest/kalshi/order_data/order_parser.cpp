...
bool read_uint64(simdjson::ondemand::object& obj, std::string_view key,
...
bool read_string(simdjson::ondemand::object& obj, std::string_view key,
                 std::string_view& out_value) noexcept {
    auto field_result = obj.find_field_unordered(key);
    if (field_result.error() != simdjson::SUCCESS) {
        return false;
    }
...
bool read_optional_string(simdjson::ondemand::object& obj, std::string_view key,
                          std::string_view& out_value) noexcept {
    auto field_result = obj.find_field_unordered(key);
    if (field_result.error() != simdjson::SUCCESS) {
        return false;
...
bool read_object(simdjson::ondemand::object& obj, std::string_view key,
                 simdjson::ondemand::object& out_value) noexcept {
    auto field_result = obj.find_field_unordered(key);
    if (field_result.error() != simdjson::SUCCESS) {
        return false;
    }
...
bool parse_scaled_fp( // NOLINT
    ...
    std::size_t decimal_places, bool allow_negative, std::int64_t& out) noexcept {
...
bool parse_money_ticks(std::string_view value, exchange::kalshi::MoneyTickRounding rounding,
                       std::int64_t& out) noexcept {

    const auto parsed = exchange::kalshi::parse_money_ticks(value, rounding);
    if (!parsed.has_value()) {
        return false;
    }
    out = *parsed;
...
bool assign_optional_client_order_id(std::string_view client_order_id,
                                     ParsedOrderMessage& out_message) noexcept {
    return client_order_id.empty() ||
           out_message.order_event.client_order_id.assign_from(client_order_id);
}
...
OrderIncomingMessageKind
OrderParser::classify_message(std::span<const std::byte> message) noexcept {
    if (message.empty()) {
...
OrderParseCode OrderParser::parse_control_response(std::span<const std::byte> message,
                                                   OrderControlResponse& out_response) noexcept {
...
OrderParseCode OrderParser::parse_fill_event(simdjson::ondemand::object& msg,
                                             ParsedOrderMessage& out_message) noexcept {
    ...
    if (!read_string(msg, "order_id", order_id) ||
        !assign_exchange_order_id(order_id, out_message)) {
        return OrderParseCode::kINVALID_ORDER_ID;
    }
    ...
    if (!read_string(msg, "yes_price_dollars", price) ||
        !parse_price_ticks(price, out_message.order_event.last_fill_price_ticks)) {
        return OrderParseCode::kMISSING_FIELD;
    }
    normalize_price_for_outcome(out_message.order_event.outcome,
                                out_message.order_event.last_fill_price_ticks);

    std::string_view count{};
    if (!read_string(msg, "count_fp", count) ||
        !parse_qty_lots(count, out_message.order_event.last_fill_qty_lots)) {
        return OrderParseCode::kMISSING_FIELD;
    }
...
OrderParseCode OrderParser::parse_user_order_event(simdjson::ondemand::object& msg,
                                                   ParsedOrderMessage& out_message) noexcept {
    ...
    if (!read_string(msg, "status", status) ||
        !parse_order_state(status, out_message.order_event.order_state)) {
    ...
    if (read_string(msg, "side", side)) {
        ...
        out_message.order_event.outcome =
            is_yes ? oms::intent::Outcome::kYES : oms::intent::Outcome::kNO;
    }

    std::string_view fill_count{};
    if (!read_string(msg, "fill_count_fp", fill_count) ||
        !parse_qty_lots(fill_count, out_message.order_event.cumulative_filled_qty_lots)) {
        return OrderParseCode::kMISSING_FIELD;
    }

    std::string_view remaining_count{};
    if (!read_string(msg, "remaining_count_fp", remaining_count) ||
        !parse_qty_lots(remaining_count, out_message.order_event.leaves_qty_lots)) {
        return OrderParseCode::kMISSING_FIELD;
    }

    std::string_view initial_count{};
    if (!read_string(msg, "initial_count_fp", initial_count) ||
        !parse_qty_lots(initial_count, out_message.order_event.ordered_qty_lots)) {
        return OrderParseCode::kMISSING_FIELD;
    }

    std::string_view client_order_id{};
    if (read_optional_string(msg, "client_order_id", client_order_id) &&
        !assign_optional_client_order_id(client_order_id, out_message)) {
        return OrderParseCode::kINVALID_ORDER_ID;
    }
...
OrderParseCode OrderParser::parse_market_position_event(simdjson::ondemand::object& msg,
                                                        ParsedOrderMessage& out_message) noexcept {
    ...
    if (!read_string(msg, "position_fp", position) ||
        ...
        !parse_money_ticks(position_cost, exchange::kalshi::MoneyTickRounding::kAWAY_FROM_ZERO,
                           out_message.order_event.position_cost_ticks) ||
        !read_string(msg, "realized_pnl_dollars", realized_pnl) ||
        !parse_money_ticks(realized_pnl, exchange::kalshi::MoneyTickRounding::kFLOOR,
                           out_message.order_event.realized_pnl_ticks) ||
        !read_string(msg, "fees_paid_dollars", fees_paid) ||
        !parse_money_ticks(fees_paid, exchange::kalshi::MoneyTickRounding::kAWAY_FROM_ZERO,
                           out_message.order_event.fees_paid_ticks) ||
        !read_string(msg, "position_fee_cost_dollars", position_fee_cost) ||
        !parse_money_ticks(position_fee_cost, exchange::kalshi::MoneyTickRounding::kAWAY_FROM_ZERO,
...
OrderParseCode OrderParser::parse_order_event(std::span<const std::byte> message,
                                              ParsedOrderMessage& out_message) noexcept {
...
OrderParseCode OrderParser::parse_message(std::span<const std::byte> message,
                                          ParsedOrderMessage& out_message) noexcept {
    out_message = ParsedOrderMessage{};
