...
bool KalshiOrderSession::emit_or_defer_oms_event(oms::KalshiToOmsEvent event) noexcept {
    ...
    (void)push_control_status(
        core::control::PrivateOrderFeedFaulted{.error_message = status_.last_error});
    // if we can't push to OMS, we need to close the websocket session to avoid further events and
    // to signal to control that we are faulted
    ws_session_.close();
...
void KalshiOrderSession::flush_deferred_oms_events() noexcept {
    while (pending_oms_count_ > 0 && oms_queues_.private_ws_to_oms_queue.try_push(
                                         std::move(pending_oms_events_[pending_oms_tail_]))) {
        pending_oms_tail_ = (pending_oms_tail_ + 1) % pending_oms_events_.size();
        --pending_oms_count_;
    }
}

void KalshiOrderSession::apply_universe_snapshot(
    ...
    (void)push_control_status(
        core::control::PrivateOrderFeedUniverseApplied{.version = desired_universe_->version});
}
...
bool KalshiOrderSession::stamp_market_route(ParsedOrderMessage& parsed) noexcept {
    ...
    if (iter == route_by_ticker_.end()) {
        parsed.order_event.market_id =
            0; /* Current Market not in this universe but OMS might be able to match leave as zero*/
        return true;
    }
...
bool KalshiOrderSession::push_control_status(
    core::control::PrivateOrderFeedToControlStatus status) noexcept {
...
bool KalshiOrderSession::connect() {
    ...
    (void)push_control_status(
        core::control::PrivateOrderFeedDisconnected{.reason = status_.last_error});
...
bool KalshiOrderSession::subscribe_channel(exchange::kalshi::KalshiOrderDataChannel channel) {
    if (active_subscriptions_.find(channel) != active_subscriptions_.end()) {
        status_.last_error = "cannot subscribe to channel - already subscribed: " +
    ...
    if (send_status != exchange::kalshi::SendStatus::kACCEPTED) {
        status_.last_error =
            std::string{"subscribe send failed: "} + std::string(ws_session_.last_error());
    ...
    pending_ws_commands_[ws_command_id] =
        PendingOrderWsCommand{.ws_command_id = ws_command_id,
...
bool KalshiOrderSession::unsubscribe_channel(exchange::kalshi::KalshiOrderDataChannel channel) {
    ...
    if (sub_it == active_subscriptions_.end()) {
        status_.last_error = "cannot unsubscribe from channel - not currently subscribed: " +
                             std::to_string(static_cast<std::uint8_t>(channel));
    ...
    auto msg = order_data_handler_.build_unsubscribe_message(ws_command_id, sub_it->second.channel,
                                                             session_ids);
    const auto send_status = ws_session_.send_text(std::move(msg));

    if (send_status != exchange::kalshi::SendStatus::kACCEPTED) {
        status_.last_error =
            std::string{"unsubscribe send failed: "} + std::string(ws_session_.last_error());
...
bool KalshiOrderSession::service_transport_once() noexcept {
    ...
    case ReadStatus::kERROR:
        status_.last_error =
            "private order feed receive failed: " + std::string{ws_session_.last_error()};
    ...
    if (code == OrderParseCode::kINVALID_JSON || code == OrderParseCode::kMISSING_FIELD ||
        code == OrderParseCode::kUNKNOWN_ORDER_STATE || code == OrderParseCode::kINVALID_ORDER_ID) {
    ...
    case OrderWsCommandKind::kSUBSCRIBE: {
        ...
        for (const auto channel : desired_channels_) {
            ...
            if (iter == active_subscriptions_.end() ||
                iter->second.phase != OrderSubscriptionPhase::kSUBSCRIBED ||
    ...
    case OrderWsCommandKind::kUNSUBSCRIBE: {
        ...
        for (const auto& [channel, sub] : active_subscriptions_) {
            ...
            if (sub.phase == OrderSubscriptionPhase::kSUBSCRIBED ||
                ...
                sub.phase == OrderSubscriptionPhase::kUNSUBSCRIBE_PENDING || sub.sid.has_value()) {
...
void KalshiOrderSession::handle_control_command(
    const core::control::ControlToPrivateOrderFeedCommand& cmd) {
    std::visit(
        [&](auto&& cmd) {
...
void KalshiOrderSession::run(const std::stop_token& stop_token) {
    while (!stop_token.stop_requested()) {
        ...
        if (status_.connected) {
            transport_progressed = service_transport_once();
        }
