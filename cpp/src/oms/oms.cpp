...
constexpr std::uint64_t kPORTFOLIO_RECONCILIATION_RETRY_NS = 1'000'000'000;

[[nodiscard]] bool
valid_timestamp_chain(const predex::oms::intent::IntentContext& context) noexcept {

    return context.ingress_timestamp_ns != 0 &&
           context.ingress_timestamp_ns <= context.book_apply_timestamp_ns &&
           context.book_apply_timestamp_ns <= context.observation_publish_timestamp_ns &&
           context.observation_publish_timestamp_ns <= context.strategy_dequeue_timestamp_ns &&
           context.strategy_dequeue_timestamp_ns <= context.evaluation_complete_timestamp_ns &&
           context.evaluation_complete_timestamp_ns <= context.intent_publish_timestamp_ns;
}

[[nodiscard]] std::optional<std::uint64_t>
full_notional_reservation(std::int64_t quantity_lots) noexcept {

    if (quantity_lots <= 0) {
        return std::nullopt;
    }

    const auto unsigned_quantity = static_cast<std::uint64_t>(quantity_lots);

    if (unsigned_quantity > std::numeric_limits<std::uint64_t>::max() / kPRICE_TICKS_PER_DOLLAR) {
        return std::nullopt;
    }

    const std::uint64_t product = unsigned_quantity * kPRICE_TICKS_PER_DOLLAR;

    return product / kQUANTITY_LOTS_PER_CONTRACT +
           static_cast<std::uint64_t>(product % kQUANTITY_LOTS_PER_CONTRACT != 0);
}

[[nodiscard]] std::optional<std::uint64_t> absolute_lots(std::int64_t quantity_lots) noexcept {
    if (quantity_lots == std::numeric_limits<std::int64_t>::min()) {
        return std::nullopt;
    }
    return static_cast<std::uint64_t>(quantity_lots < 0 ? -quantity_lots : quantity_lots);
}

[[nodiscard]] std::optional<std::uint64_t>
inventory_full_notional_exposure(std::uint64_t absolute_quantity_lots) noexcept {
    ...
    return full_notional_reservation(static_cast<std::int64_t>(absolute_quantity_lots));
}

[[nodiscard]] std::optional<std::int64_t> monetary_ticks(std::uint64_t quantity_lots,
                                                         std::int64_t price_ticks) noexcept {
    if (price_ticks < 0 ||
        quantity_lots > static_cast<std::uint64_t>(std::numeric_limits<std::int64_t>::max()) /
                            static_cast<std::uint64_t>(price_ticks == 0 ? 1 : price_ticks)) {
        return std::nullopt;
    }
    const auto product = static_cast<std::int64_t>(quantity_lots) * price_ticks;
    return product / static_cast<std::int64_t>(kQUANTITY_LOTS_PER_CONTRACT);
}

[[nodiscard]] std::optional<std::int64_t> signed_yes_lots(predex::oms::intent::Outcome outcome,
                                                          predex::oms::intent::OrderAction action,
                                                          std::int64_t quantity_lots) noexcept {
    if (quantity_lots <= 0 || outcome == predex::oms::intent::Outcome::kUNKNOWN ||
        action == predex::oms::intent::OrderAction::kUNKNOWN) {
        return std::nullopt;
    }
    const bool positive = (outcome == predex::oms::intent::Outcome::kYES &&
...
// NOLINTNEXTLINE - bugprone-exception-escape std::visit will not hit it's valueless_by_exception
std::size_t Oms::drain_strategy_intents(std::size_t max) noexcept {
    static_assert(std::is_nothrow_move_constructible_v<intent::StrategyIntent>);

    static_assert(std::is_nothrow_move_assignable_v<intent::StrategyIntent>);

    static_assert(std::is_nothrow_destructible_v<intent::StrategyIntent>);
    ...
    while (processed < max) {
        ...
        for (auto* queue : queues_.strategy_intent_queues) {
            ...
            std::visit(Overloaded{[this](const intent::NewOrderIntent& intent) noexcept {
                                      handle_strategy_intent(intent);
                                  },
                                  [this](const intent::CancelOrderIntent& intent) noexcept {
                                      handle_strategy_intent(intent);
                                  },
                                  [this](const intent::ModifyOrderIntent& intent) noexcept {
                                      handle_strategy_intent(intent);
                                  },
                                  [this](const intent::GroupOrderIntent& intent) noexcept {
                                      handle_strategy_intent(intent);
                                  }},
                       strategy_intent);
...
std::size_t
Oms::drain_venue_events(std::size_t max) noexcept { // NOLINT --bugprone-exception-escape std::visit
                                                    // will not hit it's valueless_by_exception here

    static_assert(std::is_nothrow_move_constructible_v<KalshiToOmsEvent>);
    static_assert(std::is_nothrow_move_assignable_v<KalshiToOmsEvent>);
    static_assert(std::is_nothrow_destructible_v<KalshiToOmsEvent>);
    ...
    while (processed < max) {
        ...
        for (auto* queue : queues_.venue_event_queues) {
            ...
            std::visit(Overloaded{[this](const RestOrderResponse& response) noexcept {
...
// NOLINTNEXTLINE - bugprone-exception-escape std::visit will not hit it's valueless_by_exception
std::size_t Oms::drain_control_commands(
    ...
    while (processed < max) {
        ...
        std::visit(Overloaded{[this](const core::control::ApplyOrderRouteUniverse& command) {
                                  handle_control_command(command);
                              },
                              [this](const core::control::AllowTrading& command) {
                                  handle_control_command(command);
                              },
                              [this](const core::control::DisableTrading& command) {
                                  handle_control_command(command);
                              },
                              [this](const core::control::CancelAllOrders& command) {
                                  handle_control_command(command);
                              }},
                   command);
        ++processed;
...
ClientOrderId Oms::make_client_order_id(intent::OmsRequestId oms_request_id) noexcept {
    ...
    const std::string_view identity{buffer.data(),
                                    static_cast<std::size_t>(result.ptr - buffer.data())}; // NOLINT
    (void)client_order_id.assign_from(identity);
    return client_order_id;
}

OmsContext Oms::make_context(intent::OmsRequestId oms_request_id,
                             intent::IntentContext context) noexcept {
    return OmsContext{
...
OrderStateUpdate Oms::make_order_state_update(const OrderRecord& record, VenueEventSource source,
                                              std::uint64_t update_ts_ns) const noexcept {
...
GroupRecord* Oms::find_group(std::uint16_t strategy_index, // NOLINT
                             intent::GroupIntentId group_intent_id) noexcept {

    const auto strategy_it = group_id_by_strategy_intent_.find(strategy_index);

    if (strategy_it == group_id_by_strategy_intent_.end()) {
        return nullptr;
    }

    const auto group_it = strategy_it->second.find(group_intent_id);
...
const GroupRecord* Oms::find_group(OmsGroupId oms_group_id) const noexcept {
...
const GroupRecord* Oms::find_group(std::uint16_t strategy_index, // NOLINT
...
StrategyPortfolioRecord& Oms::ensure_strategy_portfolio(std::uint16_t strategy_index) noexcept {

    auto [it, inserted] = portfolio_by_strategy_.try_emplace(strategy_index);

    if (inserted) {
        it->second = StrategyPortfolioRecord{
            .strategy_index = strategy_index,
            .next_sequence = 1,
            .allocation_limit_ticks = risk_config_.strategy_allocation_limit_ticks,
            .available_capital_ticks = risk_config_.strategy_allocation_limit_ticks,
        };
...
Oms::ensure_strategy_market_position(std::uint16_t strategy_index, intent::MarketId market_id,
                                     intent::EventId event_id) noexcept {

    auto& positions = position_by_strategy_and_market_[strategy_index];

    auto [it, inserted] = positions.try_emplace(market_id);
...
GroupAdmissionCheck
Oms::check_group_admission(const intent::GroupOrderIntent& group) const noexcept { // NOLINT
    ...
    if (active_order_universe_ == nullptr || active_order_universe_->version == 0) {
    ...
    if (group.admission_policy != intent::GroupAdmissionPolicy::kALL_OR_NONE) {
        return reject(RejectReason::kINVALID_INTENT);
    }

    if (group.context.strategy_index >= queues_.strategy_response_queues.size() ||
        queues_.strategy_response_queues[group.context.strategy_index] == nullptr) {
        return reject(RejectReason::kOMS_NOT_READY);
    }

    const auto* response_queue = queues_.strategy_response_queues[group.context.strategy_index];

    const std::size_t queued = response_queue->producer_size();

    if (queued > response_queue->capacity() ||
        response_queue->capacity() - queued < kGROUP_ADMISSION_RESPONSE_SLOTS) {
        return reject(RejectReason::kOMS_NOT_READY);
    }

    if (group.context.group_intent_id == 0 || group.context.event_id == 0 ||
        group.context.universe_version == 0 || group.context.event_revision == 0 ||
        group.context.leg_count != group.leg_count) {
        return reject(RejectReason::kINVALID_INTENT);
    }

    if (group.context.universe_version != active_order_universe_->version) {
        return reject(RejectReason::kINVALID_INTENT);
    }

    if (group.leg_count < 2 || group.leg_count > group.new_orders.size() ||
        group.leg_count > risk_config_.maximum_group_legs) {
        return reject(RejectReason::kINVALID_INTENT);
    }
    ...
    if (group.context.intent_publish_timestamp_ns > current_time_ns ||
        current_time_ns - group.context.intent_publish_timestamp_ns >
            risk_config_.maximum_group_intent_age_ns) {
        return reject(RejectReason::kINVALID_INTENT);
    }

    if (find_group(group.context.strategy_index, group.context.group_intent_id) != nullptr) {
        return reject(RejectReason::kDUPLICATE_INTENT);
    }

    if (group.expected_gross_edge_ticks < group.estimated_fee_ticks ||
        group.expected_net_edge_ticks !=
            group.expected_gross_edge_ticks - group.estimated_fee_ticks ||
    ...
    for (std::uint8_t i = 0; i < group.leg_count; ++i) {
        ...
        if (context.strategy_index != group.context.strategy_index ||
            context.event_id != group.context.event_id ||
            context.strategy_id != group.context.strategy_id ||
            context.signal_id != group.context.signal_id ||
            context.group_intent_id != group.context.group_intent_id ||
            context.universe_version != group.context.universe_version ||
            context.event_revision != group.context.event_revision ||
            context.source_shard_index != group.context.source_shard_index ||
            context.leg_index != i || context.leg_count != group.leg_count ||
            context.market_id == 0 || context.strategy_intent_id == 0) {
            return reject(RejectReason::kINVALID_INTENT);
        }

        if (leg.price_ticks <= 0 ||
            leg.price_ticks >= static_cast<std::int64_t>(kPRICE_TICKS_PER_DOLLAR) ||
            leg.quantity_lots <= 0 || leg.outcome == intent::Outcome::kUNKNOWN ||
            leg.action == intent::OrderAction::kUNKNOWN ||
            leg.liquidity_intent != intent::LiquidityIntent::kTAKER ||
            leg.order_type != intent::OrderType::kMARKETABLE_LIMIT ||
        ...
        const auto* route = find_market_route(context.market_id);
        ...
        if (!route->tradeable) {
            return reject(RejectReason::kMARKET_NOT_TRADEABLE);
        }
        ...
        for (std::uint8_t earlier = 0; earlier < i; ++earlier) {
            if (group.new_orders[earlier].context.market_id == context.market_id ||
                group.new_orders[earlier].context.strategy_intent_id ==
                    context.strategy_intent_id) {
                return reject(RejectReason::kINVALID_INTENT);
            }
        }

        const auto leg_reservation = full_notional_reservation(leg.quantity_lots);

        if (!leg_reservation.has_value() ||
            required_capital_ticks > std::numeric_limits<std::uint64_t>::max() - *leg_reservation) {
            return reject(RejectReason::kRISK_REJECTED);
        }
    ...
    if (required_capital_ticks == 0 ||
        required_capital_ticks > risk_config_.maximum_group_reservation_ticks ||
        required_capital_ticks >
            static_cast<std::uint64_t>(std::numeric_limits<std::int64_t>::max())) {
        return reject(RejectReason::kRISK_REJECTED);
    }

    std::int64_t available_capital = risk_config_.strategy_allocation_limit_ticks;

    const auto portfolio_it = portfolio_by_strategy_.find(group.context.strategy_index);

    if (portfolio_it != portfolio_by_strategy_.end()) {
        available_capital = portfolio_it->second.available_capital_ticks;
    }

    if (available_capital < 0 ||
        required_capital_ticks > static_cast<std::uint64_t>(available_capital)) {
        return reject(RejectReason::kRISK_REJECTED);
    }
    ...
    return GroupAdmissionCheck{
        ...
        .required_capital_ticks = required_capital_ticks,
    };
}

bool Oms::reserve_group_capital(GroupRecord& group) noexcept {
    ...
    auto& portfolio = ensure_strategy_portfolio(group.context.strategy_index);

    const auto reservation = static_cast<std::int64_t>(group.reserved_capital_ticks);
...
void Oms::release_group_capital(GroupRecord& group) noexcept {
    ...
    const auto released = std::min(portfolio.reserved_order_capital_ticks, reservation);
    portfolio.reserved_order_capital_ticks -= released;
    group.reserved_capital_ticks -= static_cast<std::uint64_t>(released);
...
void Oms::emit_group_admission_response(const GroupRecord& group,
    ...
    GroupAdmissionResponse response{
        ...
        .admitted_leg_count = static_cast<std::uint8_t>(
            admission_state == GroupAdmissionState::kACCEPTED ? group.leg_count : 0),
        .reserved_capital_ticks =
            admission_state == GroupAdmissionState::kACCEPTED ? group.reserved_capital_ticks : 0,
        .recv_ts_ns = recv_ts_ns,
        .response_ts_ns = now_ns(),
    };

    (void)send_strategy_message(group.context.strategy_index, OmsToStrategyMessage{response});
}

void Oms::emit_group_state_update(const GroupRecord& group, std::uint64_t update_ts_ns) noexcept {
    ...
    GroupStateUpdate update{
        ...
        .repair_attempt_count = group.repair_attempt_count,
        .residual_exposure_present = group.residual_exposure_present,
        .last_update_ts_ns = update_ts_ns,
    };

    for (std::uint8_t i = 0; i < group.leg_count; ++i) {
        const OrderRecord* order = find_order(group.original_leg_request_ids[i]);
        ...
        update.legs[i] = GroupLegState{
            .oms_request_id = order->context.oms_request_id,
            .market_id = order->market_id,
            .leg_index = order->group_leg_index,
            .order_state = order->order_state,
            .ordered_qty_lots = order->ordered_qty_lots,
            .cumulative_filled_qty_lots = order->cumulative_filled_qty_lots,
            .leaves_qty_lots = order->leaves_qty_lots,
        };
    }

    (void)send_strategy_message(group.context.strategy_index, OmsToStrategyMessage{update});
}

void Oms::emit_strategy_portfolio_update(StrategyPortfolioRecord& portfolio,
    ...
    StrategyPortfolioUpdate update{
        ...
        .allocation_limit_ticks = portfolio.allocation_limit_ticks,
        .available_capital_ticks = portfolio.available_capital_ticks,
        .reserved_order_capital_ticks = portfolio.reserved_order_capital_ticks,
        .inventory_exposure_ticks = portfolio.inventory_exposure_ticks,
        .realized_pnl_ticks = portfolio.realized_pnl_ticks,
        .fees_paid_ticks = portfolio.fees_paid_ticks,
        .venue_available_balance_ticks = venue_portfolio_.available_balance_ticks,
        .venue_portfolio_reconciled = venue_portfolio_.reconciled,
        .open_order_count = portfolio.open_order_count,
        .active_group_count = portfolio.active_group_count,
        .update_ts_ns = update_ts_ns,
    };

    (void)send_strategy_message(portfolio.strategy_index, OmsToStrategyMessage{update});
}

void Oms::emit_strategy_market_position_update(StrategyMarketPositionRecord& position,
                                               std::uint64_t update_ts_ns) noexcept {

    auto& portfolio = ensure_strategy_portfolio(position.strategy_index);
    ...
    (void)send_strategy_message(position.strategy_index, OmsToStrategyMessage{update});
}

bool Oms::portfolio_ready_for_trading() const noexcept {
    return !risk_config_.require_portfolio_reconciliation || venue_portfolio_.reconciled;
}

void Oms::recompute_strategy_available_capital(StrategyPortfolioRecord& portfolio) noexcept {
    const auto positive_reservations =
        std::max<std::int64_t>(0, portfolio.reserved_order_capital_ticks);
    const auto positive_inventory = std::max<std::int64_t>(0, portfolio.inventory_exposure_ticks);
    const auto strategy_committed =
        positive_inventory > std::numeric_limits<std::int64_t>::max() - positive_reservations
            ? std::numeric_limits<std::int64_t>::max()
            : positive_reservations + positive_inventory;
    const auto strategy_available = std::max<std::int64_t>(
        0, portfolio.allocation_limit_ticks -
               std::min(portfolio.allocation_limit_ticks, strategy_committed));
    ...
    for (const auto& [_, other] : portfolio_by_strategy_) {
        if (other.reserved_order_capital_ticks >
            std::numeric_limits<std::int64_t>::max() - global_reservations) {
            global_reservations = std::numeric_limits<std::int64_t>::max();
            break;
        }
        global_reservations += std::max<std::int64_t>(0, other.reserved_order_capital_ticks);
    }

    const auto venue_after_safety = std::max<std::int64_t>(
        0, venue_portfolio_.available_balance_ticks -
               std::min(venue_portfolio_.available_balance_ticks,
                        std::max<std::int64_t>(0, risk_config_.venue_safety_reserve_ticks)));
    const auto venue_available = std::max<std::int64_t>(
        0, venue_after_safety - std::min(venue_after_safety, global_reservations));
    portfolio.available_capital_ticks = std::min(strategy_available, venue_available);
}
...
bool Oms::maybe_request_portfolio_reconciliation() noexcept {
    if (!risk_config_.require_portfolio_reconciliation || active_order_universe_ == nullptr ||
        active_order_universe_->version == 0 || active_reconciliation_id_ != 0) {
        return false;
    }

    const auto current_time = now_ns();
    if (portfolio_reconciliation_needed_ && next_reconciliation_due_ts_ns_ != 0 &&
        current_time < next_reconciliation_due_ts_ns_) {
        return false;
    }
    const bool periodic_due = risk_config_.portfolio_reconciliation_interval_ns != 0 &&
    ...
    if (!send_kalshi_command(OmsToKalshiCommand{RequestPortfolioReconciliation{
...
void Oms::maybe_report_ready() noexcept {
    if (ready_reported_ || active_order_universe_ == nullptr ||
        active_order_universe_->version == 0 || !portfolio_ready_for_trading()) {
        return;
    }
...
void Oms::apply_venue_market_position(const VenueMarketPositionSnapshot& position,
                                      std::uint64_t update_ts_ns) noexcept {
    ...
    auto& strategy_position = ensure_strategy_market_position(0, market_id, position.event_id);
    auto& portfolio = ensure_strategy_portfolio(0);

    const auto previous_exposure = strategy_position.market_exposure_ticks;
    const auto previous_realized = strategy_position.realized_pnl_ticks;
    const auto previous_fees = strategy_position.fees_paid_ticks;

    strategy_position.net_position_lots = position.net_position_lots;
    if (position.position_cost_present) {
        strategy_position.position_cost_ticks = position.position_cost_ticks;
    }
    if (position.realized_pnl_present) {
        strategy_position.realized_pnl_ticks = position.realized_pnl_ticks;
    }
    if (position.fees_paid_present) {
        strategy_position.fees_paid_ticks = position.fees_paid_ticks;
    }

    const auto lots = absolute_lots(position.net_position_lots);
    const auto fallback_exposure =
        lots.has_value() ? inventory_full_notional_exposure(*lots) : std::nullopt;
    strategy_position.market_exposure_ticks =
        position.market_exposure_present ? position.market_exposure_ticks
    ...
    if (lots.has_value() && *lots > 0 && position.position_cost_ticks >= 0 &&
        static_cast<std::uint64_t>(position.position_cost_ticks) <=
            std::numeric_limits<std::uint64_t>::max() / kQUANTITY_LOTS_PER_CONTRACT) {
        strategy_position.average_entry_price_ticks =
            static_cast<std::int64_t>(static_cast<std::uint64_t>(position.position_cost_ticks) *
                                      kQUANTITY_LOTS_PER_CONTRACT / *lots);
    } else {
    ...
    portfolio.realized_pnl_ticks += strategy_position.realized_pnl_ticks - previous_realized;
    portfolio.fees_paid_ticks += strategy_position.fees_paid_ticks - previous_fees;
    recompute_all_strategy_available_capital();
    emit_strategy_market_position_update(strategy_position, update_ts_ns);
    emit_strategy_portfolio_update(portfolio, update_ts_ns);
}

void Oms::account_fill( // NOLINT
    OrderRecord& order, const PrivateWsOrderEvent& event) noexcept {
    const auto outcome = event.outcome == intent::Outcome::kUNKNOWN ? order.outcome : event.outcome;
    const auto action = event.action == intent::OrderAction::kUNKNOWN ? order.action : event.action;
    if (event.last_fill_qty_lots <= 0 || event.last_fill_price_ticks <= 0 ||
        event.last_fill_price_ticks >= static_cast<std::int64_t>(kPRICE_TICKS_PER_DOLLAR) ||
        outcome == intent::Outcome::kUNKNOWN || action == intent::OrderAction::kUNKNOWN) {
        return;
    }

    const auto resolved_signed_delta = signed_yes_lots(outcome, action, event.last_fill_qty_lots);
    if (!resolved_signed_delta.has_value()) {
        return;
    }
    const std::int64_t signed_delta = *resolved_signed_delta;

    if (auto* group = find_group(order.oms_group_id);
        group != nullptr && order.group_leg_index < group->leg_count) {
        auto& residual = group->residual_yes_lots[order.group_leg_index];
        if ((signed_delta > 0 &&
             residual > std::numeric_limits<std::int64_t>::max() - signed_delta) ||
            (signed_delta < 0 &&
             residual < std::numeric_limits<std::int64_t>::min() - signed_delta)) {
    ...
    auto& position = ensure_strategy_market_position(
        order.context.context.strategy_index, order.market_id, order.context.context.event_id);
    auto& portfolio = ensure_strategy_portfolio(order.context.context.strategy_index);
    ...
    const std::int64_t yes_price =
        outcome == intent::Outcome::kYES
            ...
            : static_cast<std::int64_t>(kPRICE_TICKS_PER_DOLLAR) - event.last_fill_price_ticks;
    const std::int64_t delta_side_price =
        signed_delta > 0 ? yes_price
                         : static_cast<std::int64_t>(kPRICE_TICKS_PER_DOLLAR) - yes_price;

    const bool same_direction =
        position.net_position_lots == 0 || (position.net_position_lots > 0) == (signed_delta > 0);
    const std::int64_t new_position = position.net_position_lots + signed_delta;

    if (same_direction) {
        const auto new_abs = *old_abs + *delta_abs;
        if (new_abs != 0) {
            const long double weighted =
                static_cast<long double>(*old_abs) * position.average_entry_price_ticks +
                static_cast<long double>(*delta_abs) * delta_side_price;
            position.average_entry_price_ticks =
                static_cast<std::int64_t>(weighted / static_cast<long double>(new_abs));
        }
    } else {
        ...
        const std::int64_t exit_price =
            position.net_position_lots > 0
                ...
                : static_cast<std::int64_t>(kPRICE_TICKS_PER_DOLLAR) - yes_price;
        const auto pnl =
            monetary_ticks(closed_lots, exit_price - position.average_entry_price_ticks < 0
                                            ? position.average_entry_price_ticks - exit_price
                                            : exit_price - position.average_entry_price_ticks);
        if (pnl.has_value()) {
            const bool profitable = exit_price >= position.average_entry_price_ticks;
            position.realized_pnl_ticks += profitable ? *pnl : -*pnl;
            portfolio.realized_pnl_ticks += profitable ? *pnl : -*pnl;
        }
        if (*delta_abs > *old_abs) {
            position.average_entry_price_ticks = delta_side_price;
        } else if (*delta_abs == *old_abs) {
            position.average_entry_price_ticks = 0;
        }
    }

    const auto previous_exposure = position.market_exposure_ticks;
    position.net_position_lots = new_position;
    const auto new_abs = absolute_lots(new_position);
    const auto exposure =
        new_abs.has_value() ? inventory_full_notional_exposure(*new_abs) : std::nullopt;
    position.market_exposure_ticks = exposure.has_value()
                                         ? static_cast<std::int64_t>(*exposure)
                                         : std::numeric_limits<std::int64_t>::max();
    const auto cost = new_abs.has_value()
                          ? monetary_ticks(*new_abs, position.average_entry_price_ticks)
                          : std::nullopt;
    position.position_cost_ticks = cost.value_or(0);
    portfolio.inventory_exposure_ticks += position.market_exposure_ticks - previous_exposure;

    const auto fill_reservation = full_notional_reservation(event.last_fill_qty_lots);
    if (fill_reservation.has_value()) {
        const auto released = std::min<std::int64_t>(order.reserved_capital_ticks,
                                                     static_cast<std::int64_t>(*fill_reservation));
        order.reserved_capital_ticks -= released;
        portfolio.reserved_order_capital_ticks -=
            std::min(portfolio.reserved_order_capital_ticks, released);
        if (auto* group = find_group(order.oms_group_id)) {
    ...
    portfolio_reconciliation_needed_ = risk_config_.require_portfolio_reconciliation;
}

bool Oms::group_has_residual_exposure(const GroupRecord& group) const noexcept {
    for (std::uint8_t i = 0; i < group.leg_count; ++i) {
...
bool Oms::group_has_confirmed_original_fill(const GroupRecord& group) noexcept {
    for (std::uint8_t i = 0; i < group.leg_count; ++i) {
        const auto* order = find_order(group.original_leg_request_ids[i]);
        if (order != nullptr && order->cumulative_filled_qty_lots > 0) {
            return true;
        }
    }
    return false;
...
void Oms::latch_execution_incident(std::uint64_t /*update_ts_ns*/) noexcept {
    if (!execution_incident_active_) {
...
void Oms::refresh_execution_incident_latch() noexcept {
    ...
    for (const auto& [_, group] : group_by_id_) {
        ...
        if (state == GroupExecutionState::kREPAIR_REQUIRED ||
            ...
            state == GroupExecutionState::kUNWINDING || state == GroupExecutionState::kUNCERTAIN ||
...
void Oms::maybe_report_execution_fault() noexcept {
    ...
    for (const auto& [_, group] : group_by_id_) {
        if (group.execution_state != GroupExecutionState::kUNCERTAIN &&
        ...
        if (send_control_status(core::control::OmsToControlStatus{core::control::OmsFaulted{
...
void Oms::fail_group_repair(GroupRecord& group, GroupFailureReason reason,
                            std::uint64_t update_ts_ns) noexcept {
    const bool newly_failed = group.execution_state != GroupExecutionState::kREPAIR_FAILED;
    group.execution_state = GroupExecutionState::kREPAIR_FAILED;
    group.failure_reason = reason;
    group.residual_exposure_present = group_has_residual_exposure(group);
...
void Oms::begin_group_repair(GroupRecord& group, GroupFailureReason reason,
                             std::uint64_t update_ts_ns) noexcept {
    const bool already_repairing =
        group.execution_state == GroupExecutionState::kREPAIR_REQUIRED ||
        group.execution_state == GroupExecutionState::kCANCELING_REMAINDER ||
    ...
    group.failure_reason = reason;
    group.residual_exposure_present = group_has_residual_exposure(group);
    group.execution_state = group.venue_state_uncertain ? GroupExecutionState::kUNCERTAIN
...
void Oms::continue_group_repair( // NOLINT -- suppress warning for cognitively complex function
    GroupRecord& group, std::uint64_t update_ts_ns) noexcept {
    if (group.capital_released || group.execution_state == GroupExecutionState::kREPAIR_FAILED) {
        return;
    }

    std::array<OrderRecord*, intent::kMAX_ORDERS_PER_GROUP> cancelable_orders{};
    std::size_t cancelable_count{};
    bool cancellation_pending{false};

    for (std::uint8_t i = 0; i < group.leg_count; ++i) {
        auto* original = find_order(group.original_leg_request_ids[i]);
        if (original == nullptr) {
            group.venue_state_uncertain = true;
            continue;
        }
    ...
    if (cancelable_count > 0) {
        if (group.cancel_attempt_count >= risk_config_.maximum_group_repair_attempts) {
            if (group.venue_state_uncertain) {
                ...
                fail_group_repair(group, GroupFailureReason::kCANCEL_FAILED, update_ts_ns);
            }
            return;
        }

        const std::size_t queued = queues_.kalshi_command_queue.producer_size();
        if (queued > queues_.kalshi_command_queue.capacity() ||
            queues_.kalshi_command_queue.capacity() - queued < cancelable_count) {
        ...
        for (std::size_t i = 0; i < cancelable_count; ++i) {
            if (emit_cancel_command_for_record(*cancelable_orders[i], group.context,
                                               next_oms_request_id_++, update_ts_ns)) {
                ++commands_sent;
            }
        }
    ...
    for (std::uint8_t i = 0; i < group.leg_count; ++i) {
        const auto request_id = group.active_repair_request_ids[i];
        if (request_id == 0) {
            continue;
        }
        const auto* repair = find_order(request_id);
        if (repair == nullptr || repair->order_state == OrderState::kUNCERTAIN) {
            group.venue_state_uncertain = true;
            continue;
        }
    ...
    if (group.venue_state_uncertain) {
        group.execution_state = GroupExecutionState::kUNCERTAIN;
        group.failure_reason = GroupFailureReason::kVENUE_STATE_UNCERTAIN;
        group.last_update_ts_ns = update_ts_ns;
        portfolio_reconciliation_needed_ = risk_config_.require_portfolio_reconciliation;
        emit_group_state_update(group, update_ts_ns);
        emit_telemetry();
    ...
    if (!group.residual_exposure_present) {
        const bool repaired = group_has_confirmed_original_fill(group);
        group.execution_state =
            repaired ? GroupExecutionState::kFLATTENED : GroupExecutionState::kABORTED;
        group.failure_reason = repaired ? group.failure_reason : GroupFailureReason::kNONE;
        group.last_update_ts_ns = update_ts_ns;
    ...
    if (group.repair_attempt_count >= risk_config_.maximum_group_repair_attempts) {
        fail_group_repair(group, GroupFailureReason::kUNWIND_FAILED, update_ts_ns);
        return;
    }

    std::size_t repair_count{};
    for (std::uint8_t i = 0; i < group.leg_count; ++i) {
        repair_count += static_cast<std::size_t>(group.residual_yes_lots[i] != 0);
    }
    const std::size_t queued = queues_.kalshi_command_queue.producer_size();
    if (queued > queues_.kalshi_command_queue.capacity() ||
        queues_.kalshi_command_queue.capacity() - queued < repair_count) {
        group.execution_state = GroupExecutionState::kREPAIR_REQUIRED;
        return;
    }
    ...
    for (std::uint8_t i = 0; i < group.leg_count; ++i) {
        ...
        const auto* original = find_order(group.original_leg_request_ids[i]);
        const auto quantity = absolute_lots(residual);
        if (original == nullptr || !quantity.has_value() || *quantity == 0 ||
        ...
        if (original->outcome == intent::Outcome::kYES) {
            repair_action =
                need_positive_yes ? intent::OrderAction::kBUY : intent::OrderAction::kSELL;
        } else if (original->outcome == intent::Outcome::kNO) {
            repair_action =
                need_positive_yes ? intent::OrderAction::kSELL : intent::OrderAction::kBUY;
        } else {
            group.venue_state_uncertain = true;
            break;
        }

        const auto repair_request_id = next_oms_request_id_++;
        const auto client_order_id = make_client_order_id(repair_request_id);
        if (client_order_id.empty()) {
            group.venue_state_uncertain = true;
            break;
        }
        ...
        const intent::NewOrderIntent repair_intent{
            ...
            .price_ticks = repair_action == intent::OrderAction::kBUY
                               ? static_cast<std::int64_t>(kPRICE_TICKS_PER_DOLLAR - 1)
                               : 1,
            .quantity_lots = static_cast<std::int64_t>(*quantity),
        };
        OrderRecord repair_record{
            .context = make_context(repair_request_id, context),
            .client_order_id = client_order_id,
            .order_state = OrderState::kPENDING_SUBMIT,
            .outcome = repair_intent.outcome,
            .action = repair_intent.action,
            .liquidity_intent = repair_intent.liquidity_intent,
            ...
            .pending_command_oms_request_id = repair_request_id,
            .pending_command_kind = RestCommandKind::kSUBMIT_ORDER,
            .oms_group_id = group.oms_group_id,
            .group_leg_index = i,
            .repair_order = true,
        };
        oms_request_id_to_order_record_map_.emplace(repair_request_id, repair_record);
        client_order_id_to_oms_request_id_map_.emplace(client_order_id, repair_request_id);
        group.active_repair_request_ids[i] = repair_request_id;

        if (!send_kalshi_command(OmsToKalshiCommand{SubmitOrderCmd{
            ...
            group.active_repair_request_ids[i] = 0;
            client_order_id_to_oms_request_id_map_.erase(client_order_id);
            oms_request_id_to_order_record_map_.erase(repair_request_id);
            group.venue_state_uncertain = commands_sent > 0;
            break;
        }
        ++commands_sent;
    }
    ...
    telemetry_.group_repair_commands_sent += commands_sent;
    auto& portfolio = ensure_strategy_portfolio(group.context.strategy_index);
    portfolio.open_order_count += static_cast<std::uint32_t>(commands_sent);
    group.execution_state = GroupExecutionState::kUNWINDING;
    group.last_update_ts_ns = update_ts_ns;
    emit_group_state_update(group, update_ts_ns);
    emit_strategy_portfolio_update(portfolio, update_ts_ns);
...
std::size_t Oms::service_execution_incidents() noexcept {
    ...
    for (auto& [_, group] : group_by_id_) {
        if (group.execution_state != GroupExecutionState::kREPAIR_REQUIRED &&
            group.execution_state != GroupExecutionState::kCANCELING_REMAINDER &&
            group.execution_state != GroupExecutionState::kUNWINDING) {
            continue;
        }
        const auto previous_state = group.execution_state;
        const auto previous_cancel_attempts = group.cancel_attempt_count;
        const auto previous_repair_attempts = group.repair_attempt_count;
        continue_group_repair(group, update_ts_ns);
...
void Oms::update_group_after_order_state_change( // NOLINT -- suppress warning for cognitively
                                                 // complex function
    const OrderRecord& order, std::uint64_t update_ts_ns) noexcept {
    ...
    if (order.repair_order || group->execution_state == GroupExecutionState::kREPAIR_REQUIRED ||
        group->execution_state == GroupExecutionState::kCANCELING_REMAINDER ||
        group->execution_state == GroupExecutionState::kUNWINDING ||
        group->execution_state == GroupExecutionState::kUNCERTAIN ||
        group->execution_state == GroupExecutionState::kREPAIR_FAILED) {
        ...
        if (group->execution_state != GroupExecutionState::kREPAIR_FAILED) {
            continue_group_repair(*group, update_ts_ns);
        }
        return;
    }
    ...
    for (std::uint8_t i = 0; i < group->leg_count; ++i) {
        const auto* leg = find_order(group->original_leg_request_ids[i]);
        if (leg == nullptr) {
            any_uncertain = true;
            all_filled = false;
            all_terminal = false;
        ...
        all_filled = all_filled && leg->order_state == OrderState::kFILLED;
        all_terminal = all_terminal && is_terminal(leg->order_state);
        any_fill = any_fill || leg->cumulative_filled_qty_lots > 0;
        any_uncertain = any_uncertain || leg->order_state == OrderState::kUNCERTAIN;
        any_partial = any_partial || leg->order_state == OrderState::kPARTIALLY_FILLED;
        any_terminal_failure = any_terminal_failure || (is_terminal(leg->order_state) &&
                                                        leg->order_state != OrderState::kFILLED);
        fills_fully_accounted = fills_fully_accounted && leg->reserved_capital_ticks == 0;
    }

    group->residual_exposure_present = group_has_residual_exposure(*group);
    if (any_uncertain || group->venue_state_uncertain || (all_filled && !fills_fully_accounted)) {
        begin_group_repair(*group, GroupFailureReason::kVENUE_STATE_UNCERTAIN, update_ts_ns);
    } else if (all_filled) {
        group->execution_state = GroupExecutionState::kCOMPLETED;
        ...
        group->execution_state = GroupExecutionState::kABORTED;
        group->failure_reason = GroupFailureReason::kNONE;
        group->residual_exposure_present = false;
        group->last_update_ts_ns = update_ts_ns;
        release_group_capital(*group);
        emit_group_state_update(*group, update_ts_ns);
    } else if (any_partial || any_terminal_failure || (all_terminal && any_fill)) {
        begin_group_repair(*group, GroupFailureReason::kLEG_PARTIALLY_FILLED, update_ts_ns);
    } else {
        group->execution_state =
            any_fill ? GroupExecutionState::kPARTIALLY_FILLED : GroupExecutionState::kWORKING;
        group->last_update_ts_ns = update_ts_ns;
        emit_group_state_update(*group, update_ts_ns);
    }
}

void Oms::account_terminal_order(OrderRecord& order, std::uint64_t update_ts_ns) noexcept {
    ...
    if (order.order_state != OrderState::kFILLED && order.reserved_capital_ticks > 0) {
        const auto released =
            std::min(portfolio.reserved_order_capital_ticks, order.reserved_capital_ticks);
        portfolio.reserved_order_capital_ticks -= released;
        if (auto* group = find_group(order.oms_group_id)) {
            group->reserved_capital_ticks -= std::min<std::uint64_t>(
                group->reserved_capital_ticks, static_cast<std::uint64_t>(released));
...
bool Oms::send_strategy_message(std::uint16_t strategy_index,
                                OmsToStrategyMessage message) noexcept {
...
void Oms::emit_order_state_update(const OrderRecord& record, VenueEventSource source,
                                  std::uint64_t update_ts_ns) noexcept {
    OrderStateUpdate update = make_order_state_update(record, source, update_ts_ns);
    if (send_strategy_message(record.context.context.strategy_index,
...
void Oms::emit_telemetry() noexcept {
    ...
    (void)send_control_status(
        core::control::OmsToControlStatus{core::control::OmsTelemetry{.telemetry = telemetry_}});
}
...
void Oms::handle_strategy_intent(const intent::NewOrderIntent& intent) noexcept {
    ...
    if (intent.quantity_lots <= 0 || intent.price_ticks <= 0 ||
    ...
    const auto required_capital = full_notional_reservation(intent.quantity_lots);
    if (!required_capital.has_value() ||
        *required_capital > static_cast<std::uint64_t>(std::numeric_limits<std::int64_t>::max())) {
        reject(RejectReason::kRISK_REJECTED);
        return;
    }
    auto& portfolio = ensure_strategy_portfolio(intent.context.strategy_index);
    const auto reservation = static_cast<std::int64_t>(*required_capital);
    if (portfolio.available_capital_ticks < reservation) {
        reject(RejectReason::kRISK_REJECTED);
        return;
    }
...
void Oms::handle_strategy_intent(const intent::ModifyOrderIntent& intent) noexcept {
    ...
    if (execution_incident_active_) {
        ++telemetry_.strategy_intents_rejected;
        respond(OmsResponseType::kREJECTED, RejectReason::kEXECUTION_BLOCKED);
        return;
    }
...
void Oms::handle_strategy_intent(const intent::GroupOrderIntent& intent) noexcept {
    ...
    const GroupAdmissionCheck admission = check_group_admission(intent);
    ...
    auto reject = [&](RejectReason reason) {
        ...
        const GroupRecord rejected_group{
            ...
            .execution_state = GroupExecutionState::kREJECTED,
            .leg_count = intent.leg_count,
            .last_update_ts_ns = now_ns(),
        };

        emit_group_admission_response(rejected_group, GroupAdmissionState::kREJECTED, reason,
                                      recv_ts_ns);
    };
    ...
    const OmsGroupId oms_group_id = next_oms_group_id_++;

    const intent::OmsRequestId batch_request_id = next_oms_request_id_++;
    ...
    GroupRecord group{
        ...
        .execution_state = GroupExecutionState::kADMITTED,
        .leg_count = intent.leg_count,
        .reserved_capital_ticks = admission.required_capital_ticks,
    ...
    std::array<OrderRecord, intent::kMAX_ORDERS_PER_GROUP> order_records{};

    for (std::uint8_t i = 0; i < intent.leg_count; ++i) {
        const auto& leg = intent.new_orders[i];

        const intent::OmsRequestId leg_request_id = next_oms_request_id_++;

        ClientOrderId client_order_id = make_client_order_id(leg_request_id);
        ...
        group.original_leg_request_ids[i] = leg_request_id;
        ...
        auto reserved_capital_ticks = full_notional_reservation(leg.quantity_lots).value_or(0);

        order_records[i] = OrderRecord{
            .context = make_context(leg_request_id, leg.context),
            .client_order_id = client_order_id,
            .order_state = OrderState::kPENDING_SUBMIT,
            .outcome = leg.outcome,
            .action = leg.action,
            .liquidity_intent = leg.liquidity_intent,
            .order_type = leg.order_type,
            .time_in_force = leg.time_in_force,
            .market_id = leg.context.market_id,
            .ordered_qty_lots = leg.quantity_lots,
            .working_price_ticks = leg.price_ticks,
            .leaves_qty_lots = leg.quantity_lots,
            .pending_command_oms_request_id = leg_request_id,
            .pending_command_kind = RestCommandKind::kSUBMIT_ORDER,
            .oms_group_id = oms_group_id,
            .group_leg_index = i,
            .repair_order = false,
            .reserved_capital_ticks = static_cast<std::int64_t>(reserved_capital_ticks),
        };
    }
    ...
    auto [group_it, group_inserted] = group_by_id_.emplace(oms_group_id, group);
    ...
    group_id_by_strategy_intent_[intent.context.strategy_index].emplace(
        intent.context.group_intent_id, oms_group_id);

    for (std::uint8_t i = 0; i < intent.leg_count; ++i) {
        const auto request_id = group.original_leg_request_ids[i];

        const auto& record = order_records[i];

        oms_request_id_to_order_record_map_.emplace(request_id, record);

        client_order_id_to_oms_request_id_map_.emplace(record.client_order_id, request_id);
    }

    if (!send_kalshi_command(OmsToKalshiCommand{batch_command})) {

        for (std::uint8_t i = 0; i < intent.leg_count; ++i) {
            const auto request_id = group.original_leg_request_ids[i];

            client_order_id_to_oms_request_id_map_.erase(order_records[i].client_order_id);

            oms_request_id_to_order_record_map_.erase(request_id);
        }

        auto strategy_groups_it = group_id_by_strategy_intent_.find(intent.context.strategy_index);

        if (strategy_groups_it != group_id_by_strategy_intent_.end()) {
            strategy_groups_it->second.erase(intent.context.group_intent_id);

            if (strategy_groups_it->second.empty()) {
                group_id_by_strategy_intent_.erase(strategy_groups_it);
            }
        }
    ...
    group_it->second.execution_state = GroupExecutionState::kPENDING_VENUE;
    group_it->second.last_update_ts_ns = submission_ts_ns;
    ...
    emit_group_admission_response(group_it->second, GroupAdmissionState::kACCEPTED,
                                  RejectReason::kNONE, recv_ts_ns);

    emit_group_state_update(group_it->second, submission_ts_ns);

    emit_strategy_portfolio_update(portfolio, submission_ts_ns);
}
...
void Oms::handle_venue_event(const RestOrderResponse& response) noexcept {
    ...
    if (!response.exchange_order_id.empty()) {
        ...
        exchange_order_id_to_oms_request_id_map_[response.exchange_order_id] =
            record->context.oms_request_id;
    }
    ...
    emit_order_state_update(*record, VenueEventSource::kREST_RESPONSE,
                            response.transport_recv_ts_ns);
    account_terminal_order(*record, response.transport_recv_ts_ns);
    if (!defer_group_state_updates_) {
        update_group_after_order_state_change(*record, response.transport_recv_ts_ns);
    }
}
...
void Oms::handle_venue_event(const RestOrderBatchResponse& response) noexcept {
    ...
    const std::size_t safe_order_count =
        std::min<std::size_t>(response.requested_order_count, response.order_responses.size());

    const bool previous_defer = defer_group_state_updates_;
    defer_group_state_updates_ = true;
    bool batch_correlation_missing = safe_order_count != response.requested_order_count;

    for (std::size_t i = 0; i < safe_order_count; ++i) {

        RestOrderResponse leg_response = response.order_responses[i];

        // The adapter normally stamps every leg. Treat a missing result as
        // transport uncertainty rather than silently ignoring it.
        if (leg_response.result_code == RestResultCode::kUNKNOWN) {
            leg_response.result_code = response.result_code == RestResultCode::kUNKNOWN
                                           ? RestResultCode::kTRANSPORT_ERROR
                                           : response.result_code;

            leg_response.venue_reject_reason = response.venue_reject_reason;

            if (leg_response.raw_reason_message.empty()) {
                leg_response.raw_reason_message = response.raw_reason_message;
            }
        }
    ...
    const auto* first_order = find_order(group->original_leg_request_ids[0]);
    if (batch_correlation_missing || first_order == nullptr) {
        group->venue_state_uncertain = true;
        begin_group_repair(*group, GroupFailureReason::kVENUE_STATE_UNCERTAIN,
                           response.transport_recv_ts_ns);
        return;
    }
    update_group_after_order_state_change(*first_order, response.transport_recv_ts_ns);
}
...
void Oms::handle_venue_event(const PrivateWsOrderEvent& event) noexcept {
    ...
    if (event.event_kind == PrivateWsOrderEventKind::kMARKET_POSITION) {
        ...
        apply_venue_market_position(
            VenueMarketPositionSnapshot{
                ...
                .position_fee_cost_ticks = event.position_fee_cost_ticks,
    ...
    if (!event.exchange_order_id.empty()) {
        ...
        exchange_order_id_to_oms_request_id_map_[event.exchange_order_id] =
            record->context.oms_request_id;
    }
...
void Oms::handle_venue_event( // NOLINT
    ...
    if (snapshot.result_code != PortfolioReconciliationResultCode::kCOMPLETE) {
        venue_portfolio_.reconciled = false;
        telemetry_.portfolio_reconciled = false;
        ++telemetry_.portfolio_reconciliations_failed;
        portfolio_reconciliation_needed_ = true;
        next_reconciliation_due_ts_ns_ = now_ns() + kPORTFOLIO_RECONCILIATION_RETRY_NS;
        if (trading_enabled_) {
            trading_enabled_ = false;
            emit_trading_enabled_changed();
        }
    ...
    VenuePortfolioRecord next_portfolio{
        .available_balance_ticks = snapshot.available_balance_ticks,
        .portfolio_value_ticks = snapshot.portfolio_value_ticks,
        .reconciliation_id = snapshot.reconciliation_id,
        .received_ts_ns = snapshot.received_ts_ns,
        .reconciled = true,
    };
    std::unordered_map<intent::MarketId, VenueMarketPositionSnapshot> next_positions;
    ...
    if (queues_.strategy_response_queues.size() == 1) {
        ...
        for (const auto& [market_id, venue_position] : venue_position_by_market_) {
            const auto previous_it = previous_positions.find(market_id);
            const auto previous_realized = previous_it == previous_positions.end()
                                               ? 0
                                               : previous_it->second.realized_pnl_ticks;
            const auto previous_fees =
                previous_it == previous_positions.end() ? 0 : previous_it->second.fees_paid_ticks;
            const auto resolved_position_cost =
                venue_position.position_cost_present ? venue_position.position_cost_ticks
                : previous_it == // NOLINT - readability-avoid-nested-conditional-operator concise
                        previous_positions.end()
                    ? 0
                    : previous_it->second.position_cost_ticks;
            const auto resolved_realized = venue_position.realized_pnl_present
                                               ? venue_position.realized_pnl_ticks
                                               : previous_realized;
            const auto resolved_fees =
                venue_position.fees_paid_present ? venue_position.fees_paid_ticks : previous_fees;
            const auto position_lots = absolute_lots(venue_position.net_position_lots);
            auto resolved_average_entry = previous_it == previous_positions.end()
                                              ? 0
                                              : previous_it->second.average_entry_price_ticks;
            if (venue_position.position_cost_present) {
                if (position_lots.has_value() && *position_lots > 0 &&
                    resolved_position_cost >= 0 &&
                    static_cast<std::uint64_t>(resolved_position_cost) <=
                        std::numeric_limits<std::uint64_t>::max() / kQUANTITY_LOTS_PER_CONTRACT) {
                    resolved_average_entry = static_cast<std::int64_t>(
                        static_cast<std::uint64_t>(resolved_position_cost) *
                        kQUANTITY_LOTS_PER_CONTRACT / *position_lots);
                } else {
            ...
            position = StrategyMarketPositionRecord{
                ...
                .net_position_lots = venue_position.net_position_lots,
                .average_entry_price_ticks = resolved_average_entry,
                .position_cost_ticks = resolved_position_cost,
                .market_exposure_ticks = venue_position.market_exposure_present
                                             ? venue_position.market_exposure_ticks
                                             : safe_fallback_exposure,
                .realized_pnl_ticks = resolved_realized,
                .fees_paid_ticks = resolved_fees,
            };
            portfolio.inventory_exposure_ticks += position.market_exposure_ticks;
            portfolio.realized_pnl_ticks += position.realized_pnl_ticks - previous_realized;
            portfolio.fees_paid_ticks += position.fees_paid_ticks - previous_fees;
            emit_strategy_market_position_update(position, snapshot.received_ts_ns);
        }
        ...
        for (const auto& [market_id, previous] : previous_positions) {
            ...
            auto [iter, _] =
                positions.emplace(market_id, StrategyMarketPositionRecord{
                                                 .strategy_index = 0,
                                                 .market_id = market_id,
                                                 .event_id = previous.event_id,
                                                 .realized_pnl_ticks = previous.realized_pnl_ticks,
                                                 .fees_paid_ticks = previous.fees_paid_ticks,
                                             });
            emit_strategy_market_position_update(iter->second, snapshot.received_ts_ns);
        }
    }

    telemetry_.portfolio_reconciled = true;
    telemetry_.venue_available_balance_ticks = venue_portfolio_.available_balance_ticks;
    ...
    next_reconciliation_due_ts_ns_ =
        risk_config_.portfolio_reconciliation_interval_ns == 0
            ? 0
            : now_ns() + risk_config_.portfolio_reconciliation_interval_ns;
    recompute_all_strategy_available_capital();
    for (auto& [_, portfolio] : portfolio_by_strategy_) {
        emit_strategy_portfolio_update(portfolio, snapshot.received_ts_ns);
    }
    emit_telemetry();
    maybe_report_ready();
}
...
void Oms::handle_control_command(const core::control::AllowTrading& /*command*/) noexcept {
    trading_enabled_ = !execution_incident_active_ && active_order_universe_ != nullptr &&
                       active_order_universe_->version != 0 && portfolio_ready_for_trading();
    emit_trading_enabled_changed();
}
...
void Oms::handle_control_command(const core::control::CancelAllOrders& /*command*/) noexcept {
    ...
    for (auto& [_, record] : oms_request_id_to_order_record_map_) {
        if (emit_cancel_command_for_record(record, record.context.context, next_oms_request_id_++,
                                           submission_ts)) {
    ...
    (void)send_control_status(
        core::control::OmsToControlStatus{core::control::OmsCancelAllStateChanged{
            .cancel_all_requested = cancel_all_requested_, .live_orders = live_order_count()}});
...
void Oms::handle_control_command(const core::control::ApplyOrderRouteUniverse& command) noexcept {
    const auto previous_version =
        active_order_universe_ == nullptr ? 0 : active_order_universe_->version;
    ...
    if (active_order_universe_ == nullptr || active_order_universe_->version != previous_version) {
        ...
        if (risk_config_.require_portfolio_reconciliation) {
            venue_portfolio_.reconciled = false;
            telemetry_.portfolio_reconciled = false;
            portfolio_reconciliation_needed_ = active_order_universe_ != nullptr;
            next_reconciliation_due_ts_ns_ = 0;
            recompute_all_strategy_available_capital();
        }
    }
...
bool Oms::is_terminal(OrderState state) noexcept {
    return state == OrderState::kFILLED || state == OrderState::kCANCELED ||
           state == OrderState::kREJECTED || state == OrderState::kEXPIRED;
}
...
bool Oms::is_cancelable(OrderState state) noexcept {
    return state == OrderState::kPENDING_SUBMIT || state == OrderState::kWORKING ||
           state == OrderState::kPARTIALLY_FILLED || state == OrderState::kUNCERTAIN;
}
...
const core::control::OrderMarketRoute*
Oms::find_market_route(intent::MarketId market_id) const noexcept {
...
bool Oms::emit_cancel_command_for_record(OrderRecord& record, intent::IntentContext context,
                                         intent::OmsRequestId command_oms_request_id,
                                         std::uint64_t submission_ts_ns) noexcept {
