...
nlohmann::json latency_histogram_json(const predex::utils::LatencyHistogram& histogram) {
    return {
...
nlohmann::json
channel_latency_json(const predex::core::control::MarketDataChannelLatency& latency) {
...
nlohmann::json to_json(const predex::operator_admin::OperatorResponse& response) {
    ...
    std::visit(
        [&json_response](const auto& payload) {
            ...
            if constexpr (std::is_same_v<T, predex::operator_admin::AckPayload>) {
                ...
                json_response["payload"] = {{"message", payload.message}};
                                                ...
                                                predex::operator_admin::OperatorStatusSnapshot>) {
                json_response["payload"] = {
                    {"lifecycle", lifecycle_to_string(payload.lifecycle)},
                    {"trading_session_phase",
                     trading_session_phase_to_string(payload.trading_session_phase)},
                    {"trading_enabled", payload.trading_enabled},
                    {"shutdown_requested", payload.shutdown_requested},
                };
            } else if constexpr (std::is_same_v<
                                     T, predex::operator_admin::OperatorCounterStatsSnapshot>) {
                nlohmann::json shard_stats_json = nlohmann::json::array();
                nlohmann::json shard_latency_json = nlohmann::json::array();
                predex::core::control::MarketDataChannelLatency aggregate_router_to_shard{};
                predex::core::control::MarketDataChannelLatency aggregate_shard_service{};
                predex::core::control::MarketDataChannelLatency aggregate_ingress_to_shard{};
                predex::core::control::MarketDataChannelLatency aggregate_ingress_to_book_apply{};
                for (const auto& shard_stat : payload.shard_stats) {
                    shard_stats_json.push_back({
                        ...
                        {"markets_already_awaiting_recovery",
                         shard_stat.markets_already_awaiting_recovery},
                    });
                    shard_latency_json.push_back({
                        {"shard_index", shard_stat.shard_index},
                        {"router_to_shard",
                         channel_latency_json(shard_stat.router_to_shard_latency)},
                        {"shard_service", channel_latency_json(shard_stat.shard_service_latency)},
                        {"ingress_to_shard",
                         channel_latency_json(shard_stat.ingress_to_shard_latency)},
                        {"ingress_to_book_apply",
                         channel_latency_json(shard_stat.ingress_to_book_apply_latency)},
                    });
                    for (std::size_t index = 0;
                         index < predex::core::control::kMarketDataChannelCount; ++index) {
                    ...
                    {"status_snapshot",
                     {
                         ...
                         {"trading_session_phase",
                          trading_session_phase_to_string(
                              payload.status_snapshot.trading_session_phase)},
                    ...
                    {"io_stats",
                     {
                         ...
                         {"installed_universe_version",
                          payload.io_stats.installed_universe_version},
                         {"subscribed_universe_version",
                          payload.io_stats.subscribed_universe_version},
                         ...
                         {"envelope_missing_market_ticker",
                          payload.io_stats.envelope_missing_market_ticker},
                         ...
                         {"snapshot_requests_accepted",
                          payload.io_stats.snapshot_requests_accepted},
                         {"snapshot_requests_failed", payload.io_stats.snapshot_requests_failed},
                         {"frame_pool_in_use_high_water",
                          payload.io_stats.frame_pool_in_use_high_water},
                         {"router_queue_depth_high_water",
                          payload.io_stats.router_queue_depth_high_water},
                    ...
                    {"router_stats",
                     {
                         ...
                         {"market_barriers_received",
                          payload.router_stats.market_barriers_received},
                         {"market_barriers_delivered",
                          payload.router_stats.market_barriers_delivered},
                         {"subscription_barriers_received",
                          payload.router_stats.subscription_barriers_received},
                         {"subscription_barriers_delivered",
                          payload.router_stats.subscription_barriers_delivered},
                         {"barriers_deferred", payload.router_stats.barriers_deferred},
                         {"subscription_recovery_facts_deferred",
                          payload.router_stats.subscription_recovery_facts_deferred},
                         {"shard_queue_depth_high_water",
                          payload.router_stats.shard_queue_depth_high_water},
                    ...
                    {"oms_stats",
                     {
                         ...
                         {"installed_universe_version",
                          payload.oms_stats.installed_universe_version},
                         {"unknown_market_rejects", payload.oms_stats.unknown_market_rejects},
                         {"non_tradeable_market_rejects",
                          payload.oms_stats.non_tradeable_market_rejects},
                         {"strategy_intents_received", payload.oms_stats.strategy_intents_received},
                         {"strategy_intents_processed",
                          payload.oms_stats.strategy_intents_processed},
                         ...
                         {"reconciliation_events_seen",
                          payload.oms_stats.reconciliation_events_seen},
                         {"portfolio_reconciliations_requested",
                          payload.oms_stats.portfolio_reconciliations_requested},
                         {"portfolio_reconciliations_completed",
                          payload.oms_stats.portfolio_reconciliations_completed},
                         {"portfolio_reconciliations_failed",
                          payload.oms_stats.portfolio_reconciliations_failed},
                         {"duplicate_fills_ignored", payload.oms_stats.duplicate_fills_ignored},
                         {"venue_position_updates_seen",
                          payload.oms_stats.venue_position_updates_seen},
                         {"venue_available_balance_ticks",
                          payload.oms_stats.venue_available_balance_ticks},
                         {"portfolio_reconciled", payload.oms_stats.portfolio_reconciled},
                         {"order_state_updates_sent", payload.oms_stats.order_state_updates_sent},
                         {"strategy_response_backpressure",
                          payload.oms_stats.strategy_response_backpressure},
                         {"execution_incidents_latched",
                          payload.oms_stats.execution_incidents_latched},
                         {"group_repair_attempts", payload.oms_stats.group_repair_attempts},
                         {"group_repair_commands_sent",
                          payload.oms_stats.group_repair_commands_sent},
                         {"group_repairs_completed", payload.oms_stats.group_repairs_completed},
                         {"group_repairs_failed", payload.oms_stats.group_repairs_failed},
                         {"execution_incident_active", payload.oms_stats.execution_incident_active},
                    ...
                    {"private_order_feed_stats",
                     {
                         ...
                         {"installed_universe_version",
                          payload.private_order_feed_stats.installed_universe_version},
                         {"subscribed_universe_version",
                          payload.private_order_feed_stats.subscribed_universe_version},
                         ...
                         {"oms_enqueue_failures",
                          payload.private_order_feed_stats.oms_enqueue_failures},
                    ...
                    {"order_rest_stats",
                     {
                         ...
                         {"installed_universe_version",
                          payload.order_rest_stats.installed_universe_version},
                         ...
                         {"last_portfolio_reconciliation_error",
                          payload.order_rest_stats.last_portfolio_reconciliation_error},
                    ...
                    {"recovery_stats",
                     {
                         ...
                         {"markets_already_recovering",
                          payload.recovery_stats.markets_already_recovering},
                         ...
                         {"request_enqueue_failures",
                          payload.recovery_stats.request_enqueue_failures},
                         ...
                         {"recovery_duration_samples",
                          payload.recovery_stats.recovery_duration_samples},
                         {"recovery_duration_total_ns",
                          payload.recovery_stats.recovery_duration_total_ns},
                         {"recovery_duration_max_ns",
                          payload.recovery_stats.recovery_duration_max_ns},
                     }},
                    {"latency_stats",
                     {
                         {"clock", "steady_clock"},
                         ...
                         {"io",
                          {
                              {"wire_service",
                               channel_latency_json(payload.io_stats.wire_service_latency)},
                          }},
                         {"router",
                          {
                              {"wire_to_router",
                               channel_latency_json(payload.router_stats.wire_to_router_latency)},
                              {"router_service",
                               channel_latency_json(payload.router_stats.router_service_latency)},
                          }},
                         {"shards", std::move(shard_latency_json)},
                         {"shards_aggregate",
                          {
                              {"router_to_shard", channel_latency_json(aggregate_router_to_shard)},
                              {"shard_service", channel_latency_json(aggregate_shard_service)},
                              {"ingress_to_shard",
                               channel_latency_json(aggregate_ingress_to_shard)},
                              {"ingress_to_book_apply",
                               channel_latency_json(aggregate_ingress_to_book_apply)},
                          }},
                         {"logger",
                          {
                              {"shard_to_logger",
                               channel_latency_json(payload.logger_stats.shard_to_logger_latency)},
                              {"ingress_to_logger_write",
                               channel_latency_json(
                                   payload.logger_stats.ingress_to_logger_write_latency)},
                          }},
                         {"recovery_total",
                          latency_histogram_json(payload.recovery_stats.recovery_duration_latency)},
                     }},
                };
            } else {
...
predex::socket::CommandResponse
answer_operator(const predex::operator_admin::OperatorResponse& operator_response) {
    const bool okay =
        operator_response.type != predex::operator_admin::OperatorResponseType::kERROR;
        ...
        while (std::chrono::steady_clock::now() < deadline) {
            if (queues_.control_to_server_queue.try_pop(operator_response)) {
                ...
                /*
                    ...
                    but if it does somehow we don't want to crash/stall the server so just return
                   the error back.


                    for future use cases where multiple concurrent commands are expected, will
                   necessitate a different architecture from SPSC queues.
                */
                return socket::CommandResponse{
                    .ok = false,
