...
EventId Event::event_id() const noexcept { return state_.event_id; }

EventTopology Event::event_topology() const noexcept { return state_.topology; }

std::uint32_t Event::shard_event_index() const noexcept { return state_.shard_event_index; }

bool Event::usable() const noexcept {
    return !state_.markets.empty() &&
           std::ranges::all_of(state_.markets,
                               [](const KalshiMarket& market) { return market.book.usable(); });
}

EventApplyResult Event::apply(std::uint32_t event_market_index,
    ...
    const bool is_book_message = std::holds_alternative<KalshiSnapshotEvent>(parsed_event) ||
                                 std::holds_alternative<KalshiDeltaData>(parsed_event);

    auto& market = state_.markets[event_market_index];

    const MarketApplyResult market_result = apply_to_market(market, std::move(parsed_event));

    const bool book_changed =
        is_book_message && market_result.disposition == ApplyDisposition::kAPPLIED;

    const bool sync_state_changed = market_result.book_sync_transition != BookSyncTransition::kNONE;
    ...
    const bool event_became_usable = !event_was_usable && event_is_usable;
    ...
    return EventApplyResult{
        ...
        .book_sync_transition = market_result.book_sync_transition,
        .reason = market_result.reason,
        .book_changed = book_changed,
        .event_became_usable = event_became_usable,
        .event_revision = revision_,
    };
}

std::uint64_t Event::revision() const noexcept { return revision_; }
...
MarketApplyResult Event::apply_to_market( // NOLINT - bugprone-exception-escape std::visit will not
                                          // hit it's valueless_by_exception here
    KalshiMarket& market, KalshiParsedEvent&& parsed_event) noexcept {
    ...
    return std::visit(
        [&](auto&& event) noexcept -> MarketApplyResult {
            ...
            if constexpr (std::is_same_v<T, KalshiSnapshotEvent>) {
                ...
                return MarketApplyResult{.disposition = ApplyDisposition::kREJECTED,
                                         .book_sync_transition = BookSyncTransition::kNONE,
...
MarketApplyResult Event::apply_snapshot(KalshiMarket& market,
                                        KalshiSnapshotEvent&& parsed_event) noexcept {
    ...
    return MarketApplyResult{
        ...
        .book_sync_transition =
            became_usable ?
                          // NOLINTNEXTLINE - readability-avoid-nested-conditional-operator think
                (prev_sync_state == BookSyncState::kAWAITING_INITIAL_SNAPSHOT
                     ? BookSyncTransition::kINITIAL_SNAPSHOT_INSTALLED
                     : BookSyncTransition::kRECOVERED)
                          : BookSyncTransition::kNONE,
        .reason = MarketApplyReason::kNONE};
}

MarketApplyResult Event::apply_delta(KalshiMarket& market,
                                     KalshiDeltaData&& parsed_event) noexcept {
    if (market.book.sync_state == BookSyncState::kAWAITING_INITIAL_SNAPSHOT) {
        return {.disposition = ApplyDisposition::kIGNORED,
                .book_sync_transition = BookSyncTransition::kNONE,
...
MarketApplyResult Event::apply_trade(KalshiMarket& market,
                                     KalshiTradeData&& parsed_event) noexcept {
    ...
    return MarketApplyResult{.disposition = ApplyDisposition::kAPPLIED,
                             .book_sync_transition = BookSyncTransition::kNONE,
                             .reason = MarketApplyReason::kNONE};
}

MarketApplyResult Event::apply_lifecycle(KalshiMarket& market,
                                         KalshiLifecycleData&& parsed_event) noexcept {
...
MarketApplyResult Event::reject_snapshot(KalshiMarket& market, MarketApplyReason reason) noexcept {
...
MarketApplyResult Event::reject_delta(KalshiMarket& market, MarketApplyReason reason) noexcept {
    ...
    market.book.invalidate();
    return MarketApplyResult{.disposition = ApplyDisposition::kREJECTED,
                             .book_sync_transition = was_usable
                                                         ? BookSyncTransition::kBECAME_UNUSABLE
                                                         : BookSyncTransition::kNONE,
                             .reason = reason};
}

bool Event::install_level(const KalshiBook& book, std::vector<QtyLots>& levels,
...
BookInvalidationResult
Event::invalidate_market(std::uint32_t event_market_index,
...
BookInvalidationSummary
Event::invalidate_all_markets(predex::ingest::kalshi::BookInvalidationReason reason) noexcept {
...
std::size_t Event::market_count() const noexcept { return state_.markets.size(); }
// NOLINTNEXTLINE
std::optional<strategy::MonotonicPairObservation> Event::build_monotonic_pair_observation(
    std::uint32_t easier_market_index,
    const strategy::StrategyObservationContext& context) const noexcept {
    ...
    if (state_.markets.size() < 2 || easier_market_index >= state_.markets.size() - 1) {
        return std::nullopt;
    }
    ...
    if (easier_market.event_market_index != easier_market_index ||
        ...
        !easier_market.strike_key.has_value() || !harder_market.strike_key.has_value() ||
    ...
    easier_market_view.market_expected_expiration_time_s =
        easier_market.market_expected_expiration_time_s;
    harder_market_view.market_expected_expiration_time_s =
        harder_market.market_expected_expiration_time_s;
    ...
    for (std::size_t index = easier_market.book.bids.size();
         index > 0 && bid_count < strategy::kStrategyBookDepth; --index) {
        ...
        const auto price = easier_market.book.price_ticks_at_index(book_index);
    ...
    for (std::size_t index = 0;
         index < easier_market.book.asks.size() && ask_count < strategy::kStrategyBookDepth;
         ++index) {
        ...
        const auto price = easier_market.book.price_ticks_at_index(index);
    ...
    for (std::size_t index = harder_market.book.bids.size();
        ...
        const auto price = harder_market.book.price_ticks_at_index(book_index);
    ...
    for (std::size_t index = 0;
         index < harder_market.book.asks.size() && ask_count < strategy::kStrategyBookDepth;
         ++index) {
        ...
        const auto price = harder_market.book.price_ticks_at_index(index);
    ...
    return strategy::MonotonicPairObservation{.universe_version = context.universe_version,
                                              .shard_index = context.shard_index,
                                              .event_id = event_id(),
                                              .event_revision = revision(),
                                              .ingress_timestamp_ns = context.ingress_timestamp_ns,
                                              .book_apply_timestamp_ns =
