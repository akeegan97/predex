...
EventApplyResult EventStore::apply(const ingest::kalshi::FrameHandle& handle,
                                   KalshiParsedEvent&& parsed_event) noexcept {
    if (handle.shard_event_index >= events_.size()) {
        return EventApplyResult{.disposition = ApplyDisposition::kREJECTED,
...
std::size_t EventStore::size() const noexcept { return events_.size(); }

BookInvalidationResult EventStore::invalidate_market(
    // NOLINTNEXTLINE -- easily swapped parameter suppressed
    std::uint32_t shard_event_index, std::uint32_t event_market_index, MarketId expected_market_id,
    predex::ingest::kalshi::BookInvalidationReason reason) noexcept {
    if (shard_event_index >= events_.size()) {
        return BookInvalidationResult{.target_found = false,
                                      .book_sync_transition = BookSyncTransition::kNONE,
                                      .reason = reason,
                                      .reject_reason =
                                          InvalidationRejectReason::kINVALID_EVENT_INDEX};
    }
    ...
    if (market == nullptr) {
        return BookInvalidationResult{.target_found = false,
                                      ...
                                      .reason = reason,
                                      .reject_reason =
                                          InvalidationRejectReason::kINVALID_MARKET_INDEX};
    }
    if (market->market_id != expected_market_id) {
        return BookInvalidationResult{.target_found = false,
                                      ...
                                      .reason = reason,
                                      .reject_reason =
                                          InvalidationRejectReason::kMARKET_ID_MISMATCH};
    }
    return event.invalidate_market(event_market_index, reason);
}
BookInvalidationSummary
EventStore::invalidate_all_markets(predex::ingest::kalshi::BookInvalidationReason reason) noexcept {
    BookInvalidationSummary summary{};
    for (std::size_t idx = 0; idx < events_.size(); // NOLINT -- suppressing range-based loop
         ++idx) {
