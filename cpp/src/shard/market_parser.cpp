...
bool get_int64_t(simdjson::ondemand::object& obj, std::string_view key,
...
bool get_uint64_t(simdjson::ondemand::object& obj, std::string_view key,
...
bool parse_scaled_fp(std::string_view value,
...
bool get_first_value(simdjson::ondemand::object& obj, std::initializer_list<std::string_view> keys,
                     simdjson::ondemand::value& out) noexcept {
...
bool parse_delta_qty_value(simdjson::ondemand::value& value,
                           predex::shard::DeltaQtyLots& out) noexcept {
...
bool parse_first_price_value(simdjson::ondemand::object& msg,
                             std::initializer_list<std::string_view> keys,
                             predex::shard::PriceTicks& out) noexcept {
    simdjson::ondemand::value value{};
    return get_first_value(msg, keys, value) && parse_price_value(value, out);
}

bool parse_first_qty_value(simdjson::ondemand::object& msg,
                           std::initializer_list<std::string_view> keys,
                           predex::shard::QtyLots& out) noexcept {
    simdjson::ondemand::value value{};
    return get_first_value(msg, keys, value) && parse_qty_value(value, out);
}

bool parse_first_delta_qty_value(simdjson::ondemand::object& msg,
                                 std::initializer_list<std::string_view> keys,
                                 predex::shard::DeltaQtyLots& out) noexcept {
    simdjson::ondemand::value value{};
...
bool parse_levels(simdjson::ondemand::array& arr, std::vector<predex::shard::Level>& out,
                  bool reciprocal_price_enabled) noexcept {
...
OptionalLevelsParse parse_optional_levels(simdjson::ondemand::object& msg, std::string_view key,
...
bool parse_snapshot(simdjson::ondemand::object& msg,
                    predex::shard::KalshiSnapshotEvent& out) noexcept {
    ...
    if (yes_levels == OptionalLevelsParse::kMISSING) {
        ...
        if (get_array(msg, "yes", legacy_yes_levels) &&
            !parse_levels(legacy_yes_levels, out.bids, false)) {
            return false;
        }
    }
    if (no_levels == OptionalLevelsParse::kMISSING) {
        simdjson::ondemand::array legacy_no_levels{};
        if (get_array(msg, "no", legacy_no_levels) &&
            !parse_levels(legacy_no_levels, out.asks, true)) {
            return false;
        }
    }
    return true;
...
bool parse_trade(simdjson::ondemand::object& msg, predex::shard::KalshiTradeData& out) noexcept {
    if (!parse_first_price_value(msg, {"price_dollars", "yes_price_dollars", "price", "yes_price"},
                                 out.price_ticks)) {
        return false;
    }
...
bool parse_lifecycle(simdjson::ondemand::object& msg,
                     predex::shard::KalshiLifecycleData& out) noexcept {
...
ParseResult MarketParser::parse(const ingest::kalshi::FrameHandle& handle,
                                const ingest::kalshi::KalshiFrame& frame,
    ...
    simdjson::padded_string_view json{reinterpret_cast<const char*>(frame.payload.data()),
                                      frame.len, frame.payload.size()};
