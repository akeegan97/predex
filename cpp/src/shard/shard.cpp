...
Shard::Shard(std::uint32_t shard_index, ShardQueues queues,
...
bool Shard::send_or_defer_recovery_status(ShardToControlMessage message) noexcept {
...
bool Shard::report_market_recovery_required(
    std::uint64_t universe_version, const ingest::kalshi::IntegrityIncidentKey& incident,
    std::uint32_t sid, std::uint64_t sequence, MarketId market_id, EventId event_id,
    ingest::kalshi::BookInvalidationReason reason) noexcept {
    return send_or_defer_recovery_status(ShardToControlMessage{ShardMarketRecoveryRequired{
...
bool Shard::command_matches_shard(std::uint64_t universe_version, // NOLINT
                                  std::uint32_t command_shard_index) noexcept {
...
std::uint32_t Shard::shard_index() const noexcept { return shard_index_; }

const ShardStats& Shard::stats() const noexcept { return stats_; }

bool Shard::terminal_handoff(predex::ingest::kalshi::FrameHandle handle) noexcept {
    // TODO: change return from bool to enum to indicate if handle was recycled or sent to logger on
    // true, or if handle was leaked on false
    ...
    if (channel_index < core::control::kMarketDataChannelCount) {
        utils::record_elapsed_ns(stats_.router_to_shard_latency[channel_index],
                                 handle.router_publish_ts_ns, handle.shard_dequeue_ts_ns);
        utils::record_elapsed_ns(stats_.shard_service_latency[channel_index],
                                 handle.shard_dequeue_ts_ns, handle.shard_publish_ts_ns);
        utils::record_elapsed_ns(stats_.ingress_to_shard_latency[channel_index],
                                 handle.ingress_ts_ns, handle.shard_publish_ts_ns);
    }
...
ShardPumpResult Shard::drain_one_message() noexcept {
    ...
    if (!queues_.router_to_shard_queue.try_pop(message)) {
        ...
        ShardDrainComplete drained{.universe_version = installed_universe_version_,
                                   .shard_index = shard_index_,
                                   .stats = stats_};
...
ShardPumpResult Shard::pump_once() noexcept {
    ...
    if (run_state_ == ShardRunState::kUNINSTALLED || run_state_ == ShardRunState::kDRAINED ||
...
ShardPumpResult Shard::dispatch_message(const ingest::kalshi::MarketDataPathMessage& message,
                                        MarketDataDispatchMode dispatch_mode) noexcept {

    assert(!message.valueless_by_exception());

    return std::visit(
        [this, dispatch_mode](const auto& msg) noexcept -> ShardPumpResult {
...
ShardPumpResult Shard::handle_message(const ingest::kalshi::FrameHandle& incoming_handle,
                                      MarketDataDispatchMode dispatch_mode) noexcept {
    ShardPumpResult result{};
    auto handle = incoming_handle;
    handle.shard_dequeue_ts_ns = utils::monotonic_now_ns();
    const bool is_order_book =
        handle.kind == predex::ingest::kalshi::FrameKind::kORDERBOOK_DELTA ||
        handle.kind == predex::ingest::kalshi::FrameKind::kORDERBOOK_SNAPSHOT;
    ++stats_.frames_seen;
    if (dispatch_mode == MarketDataDispatchMode::kDRAINING) {
        ShardPumpResult result{};

        result.code =
            terminal_handoff(handle) ? ShardPumpCode::kDRAINED_FRAME : ShardPumpCode::kHANDLE_LEAK;
    ...
    if (frame == nullptr) {
        ...
        if (is_order_book) {
            result.market_invalidation = event_store_.invalidate_market(
                handle.shard_event_index, handle.event_market_index, handle.market_id,
                ingest::kalshi::BookInvalidationReason::kSHARD_FRAME_MISSING);
            if (!account_invalidation_result(result.market_invalidation)) {
                fault_shard(handle.universe_version,
                ...
                if (result.market_invalidation.book_sync_transition ==
                    BookSyncTransition::kBECAME_UNUSABLE) {
                    publish_event_unavailable(handle.universe_version, handle.event_id,
                                              handle.market_id,
            ...
            if (result.market_invalidation.book_sync_transition ==
                ...
                if (!report_market_recovery_required(
                        handle.universe_version, result.incident, handle.sid, handle.sequence,
                        handle.market_id, handle.event_id,
                        ingest::kalshi::BookInvalidationReason::kSHARD_FRAME_MISSING)) {
                    fault_shard(handle.universe_version,
    ...
    if (is_order_book) {
        ...
        if (event_result.disposition == ApplyDisposition::kAPPLIED &&
            ...
            utils::record_elapsed_ns(stats_.ingress_to_book_apply_latency[channel_index],
                                     handle.ingress_ts_ns, apply_complete_ts_ns);
        }
    }
    ...
    if (event_result.book_sync_transition == BookSyncTransition::kBECAME_UNUSABLE) {
        publish_event_unavailable(handle.universe_version, handle.event_id, handle.market_id,
    ...
    if (event_result.disposition == ApplyDisposition::kAPPLIED) {
        ...
        if (event_result.book_changed) {
            publish_affected_pairs(handle, event_result, apply_complete_ts_ns);
        }
        if (handle.recovery_id != 0 &&
            ...
            if (!send_or_defer_recovery_status(ShardToControlMessage{ShardRecoverySnapshotApplied{
                ...
                fault_shard(handle.universe_version, "Could not retain recovery-snapshot status");
            }
        }
        if (!terminal_handoff(handle)) {
            result.code = ShardPumpCode::kHANDLE_LEAK;
...
ShardPumpResult Shard::handle_message(const ingest::kalshi::MarketInvalidationBarrier& message,
                                      MarketDataDispatchMode dispatch_mode) noexcept {
    ...
    const auto reject_barrier = [this, &message](const char* reason) {
        ...
        (void)send_control_message(ShardToControlMessage{ShardFaulted{
            .shard_index = shard_index_,
    ...
    const BookInvalidationResult invalidation = event_store_.invalidate_market(
        message.shard_event_index, message.event_market_index, message.market_id, message.reason);
    ...
    if (!account_invalidation_result(invalidation)) {
        auto result = reject_barrier("Invalid transition returned by market invalidation");
        result.market_invalidation = invalidation;
        return result;
    }

    if (invalidation.book_sync_transition == BookSyncTransition::kBECAME_UNUSABLE) {
        publish_event_unavailable(message.universe_version, message.event_id, message.market_id,
                                  invalidation.event_revision);
    }

    if (invalidation.book_sync_transition == BookSyncTransition::kBECAME_UNUSABLE ||
        invalidation.book_sync_transition == BookSyncTransition::kRECOVERY_REQUIRED) {
        if (!report_market_recovery_required(message.universe_version, message.incident,
                                             message.sid, message.sequence, message.market_id,
                                             message.event_id, message.reason)) {
            return reject_barrier("Could not retain market recovery status");
        }
    }
...
ShardPumpResult
Shard::handle_message(const ingest::kalshi::OrderBookSubscriptionInvalidationBarrier& message,
                      MarketDataDispatchMode dispatch_mode) noexcept {
    std::ignore = dispatch_mode;
...
bool Shard::process_one_control_command() {
    ...
    std::visit([&](auto& cmd) { handle_operator_command(cmd); }, command);
    return true;
}
...
void Shard::handle_operator_command(InstallShardUniverse& command) {
    ...
    const bool install_allowed =
        run_state_ == ShardRunState::kUNINSTALLED || run_state_ == ShardRunState::kDRAINED;

    if (!install_allowed) {
        fault_shard(command.universe_version,
    ...
    if (run_state_ == ShardRunState::kDRAINED &&
        ...
        fault_shard(command.universe_version, "Replacement universe version is not newer");
        return;
    }

    if (!install_universe(std::move(command.events))) {
        fault_shard(command.universe_version, "Failed to install universe events");
        return;
    }
    ...
    ShardUniverseInstalled installed{.universe_version = command.universe_version,
                                     .shard_index = shard_index_,
...
void Shard::handle_operator_command(PrepareStopUniverse& command) {
    if (!command_matches_shard(command.universe_version, command.shard_index) ||
        !require_run_state(ShardRunState::kLIVE, command.universe_version,
                           "Prepare stop requested before Shard was live")) {
        return;
    }
    run_state_ = ShardRunState::kPREPARING_STOP;
    run_state_ = ShardRunState::kSAFE_TO_STOP;
    ShardSafeToStopUniverse safe_to_stop{.universe_version = command.universe_version,
                                         .shard_index = shard_index_,
...
void Shard::handle_operator_command(DrainShardUniverse& command) {
    if (!command_matches_shard(command.universe_version, command.shard_index) ||
        !require_run_state(ShardRunState::kSAFE_TO_STOP, command.universe_version,
                           "Drain requested before Shard was safe to stop")) {
        return;
    }
...
void Shard::handle_operator_command(ResumeShardUniverse& command) {
    if (!command_matches_shard(command.universe_version, command.shard_index) ||
        !require_run_state(ShardRunState::kSAFE_TO_STOP, command.universe_version,
                           "Resume requested before Shard was safe to stop")) {
        return;
    }
    run_state_ = ShardRunState::kLIVE;
}
void Shard::maybe_send_telemetry() noexcept {
    ...
    (void)send_control_message(ShardToControlMessage{ShardTelemetry{
        .shard_index = shard_index_,
...
bool Shard::require_run_state(ShardRunState expected, std::uint64_t universe_version,
...
void Shard::fault_shard(std::uint64_t universe_version, const char* reason) noexcept {
    ...
    ShardFaulted faulted{
        .shard_index = shard_index_, .universe_version = universe_version, .reason = reason};
...
bool Shard::publish_strategy_message( // NOLINT - bugprone-exception-escape --> std::visit will not
                                      // encounter
...
void Shard::publish_event_unavailable(std::uint64_t universe_version, EventId event_id,
                                      MarketId affected_market_id,
                                      std::uint64_t event_revision) noexcept { // NOLINT

    (void)publish_strategy_message(
        strategy::ShardToStrategyMessage{strategy::StrategyEventUnavailable{
            .universe_version = universe_version,
            .shard_index = shard_index_,
            .event_id = event_id,
...
void Shard::publish_shard_unavailable(std::uint64_t universe_version) noexcept {
    (void)publish_strategy_message(
        strategy::ShardToStrategyMessage{strategy::StrategyShardUnavailable{
            .universe_version = universe_version,
            .shard_index = shard_index_,
        }});
}

void Shard::publish_affected_pairs(const ingest::kalshi::FrameHandle& handle,
                                   const EventApplyResult& apply_result,
                                   std::uint64_t apply_complete_ts_ns) noexcept {
    if (strategy_publication_faulted_ || !apply_result.book_changed ||
    ...
    const auto publish_pair = [this, event, &handle,
                               apply_complete_ts_ns](std::uint32_t easier_index) noexcept -> bool {
        ...
        return publish_strategy_message(strategy::ShardToStrategyMessage{*observation});
    };
