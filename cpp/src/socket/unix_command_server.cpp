...
UnixCommandServer::UnixCommandServer(OperatorSocketConfig config, ICommandHandler* command_handler)
    : config_(std::move(config)), command_handler_(command_handler) {}

// NOLINTNEXTLINE
void UnixCommandServer::run(const std::stop_token& stop_token, std::string& error_out,
                            const ServerReadyCallback& on_ready) {
    ...
    UniqueFD lock_fd(
        ::open(lock_path.c_str(), O_CREAT | O_RDWR | O_CLOEXEC | O_NOFOLLOW, 0600)); // NOLINT
    ...
    if (::lstat(config_.socket_path.c_str(), &stat_t) == 0) {
        ...
        if (stat_t.st_uid != ::geteuid()) {
            error_out =
                "socket path exists and is not owned by the current user: " + config_.socket_path;
            return;
        }
        if (::unlink(config_.socket_path.c_str()) < 0) {
            error_out =
                "failed to remove existing socket file: " + std::string(std::strerror(errno));
            return;
        }
    } else {
        if (errno != ENOENT) {
    ...
    std::stop_callback stop_callback{stop_token, [&stop_fd] {
                                         const std::uint64_t value = 1;
                                         for (;;) {
                                             const ssize_t ret =
                                                 ::write(stop_fd.get(), &value, sizeof(value));
    ...
    for (;;) {
        ...
        std::array<pollfd, 2> fds{{{.fd = listen_fd.get(), .events = POLLIN, .revents = 0},
        ...
        if ((fds[0].revents & POLLIN) != 0) {
            ...
            UniqueFD client_fd(
                ::accept4(listen_fd.get(), nullptr, nullptr, SOCK_NONBLOCK | SOCK_CLOEXEC));
            ...
            while (!request_complete && !abandon_client) {
                ...
                const auto remaining =
                    std::chrono::ceil<std::chrono::milliseconds>(read_deadline - now);

                const int timeout_ms = static_cast<int>(remaining.count());

                std::array<pollfd, 2> inner_fds{
                    {{.fd = client_fd.get(), .events = POLLIN, .revents = 0},
                     {.fd = stop_fd.get(), .events = POLLIN, .revents = 0}}};

                const int inner_ret =
                    ::poll(inner_fds.data(), static_cast<nfds_t>(inner_fds.size()), timeout_ms);
                ...
                if (inner_ret < 0) {
                    ...
                    error_out =
                        "client read poll failed: " + std::string(std::strerror(poll_error));
                    return;
                }
                ...
                if ((inner_fds[0].revents & (POLLIN | POLLHUP | POLLERR)) != 0) {
                    ...
                    for (;;) {
                        ...
                        if (bytes_read > 0) {
                            ...
                            if (request.size() + payload_count > config_.max_request_bytes) {
                                abandon_client = true;
                                break;
                            }
            ...
            std::array<struct pollfd, 2> write_fds{
                {{.fd = client_fd.get(), .events = POLLOUT, .revents = 0},
                 {.fd = stop_fd.get(), .events = POLLIN, .revents = 0}}};
            ...
            while (bytes_written_total < response.body.size()) {
                ...
                const auto remaining =
                    std::chrono::ceil<std::chrono::milliseconds>(write_deadline - now);
                const int write_timeout_ms = static_cast<int>(remaining.count());

                write_fds[0].revents = 0;
                write_fds[1].revents = 0;

                const int write_ret = ::poll(
                    write_fds.data(), static_cast<nfds_t>(write_fds.size()), write_timeout_ms);
                ...
                if (write_ret < 0) {
                    ...
                    const int poll_error = errno;
                    error_out =
                        "client write poll failed: " + std::string(std::strerror(poll_error));
                    return;
                }
                ...
                const ssize_t bytes_written =
                    ::send(client_fd.get(), response.body.data() + bytes_written_total,
                           response.body.size() - bytes_written_total, MSG_NOSIGNAL);
                if (bytes_written > 0) {
