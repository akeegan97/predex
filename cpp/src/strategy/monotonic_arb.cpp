...
namespace{
    ...
    struct PairedSelectionResult{
        ...
        MonotonicArbRejectReason failure{
            MonotonicArbRejectReason::kNONE
        };
    };

    [[nodiscard]] MonotonicArbEvaluation reject(
    MonotonicArbRejectReason reason) noexcept {
    ...
    [[nodiscard]] bool valid_pair_header(
        const MonotonicPairObservation& observation) noexcept {

        if (observation.event_id == 0 ||
            observation.easier.market_id == 0 ||
            observation.harder.market_id == 0) {
            return false;
        }

        if (observation.easier.market_id ==
            observation.harder.market_id) {
            return false;
        }

        if (observation.easier.strike_key >=
            observation.harder.strike_key) {
            return false;
        }

        if (observation.easier.event_market_index >=
            observation.harder.event_market_index) {
            return false;
        }

        if (observation.harder.event_market_index -
                observation.easier.event_market_index != 1) {
            return false;
        }
    ...
    [[nodiscard]] QtyLots add_up_to_required(QtyLots current,QtyLots additional, QtyLots required) noexcept {//NOLINT
...
//NOLINTNEXTLINE(readability-function-cognitive-complexity) -- complex as all logic is inline for ease of reasoning around
    [[nodiscard]] LegCandidateSet collect_easier_candidates(
        const StrategyMarketView& market,
        const MonotonicArbConfig& config) noexcept {

        if (market.ask_count == 0 ||
            market.ask_count > predex::strategy::kStrategyBookDepth) {
            return candidate_failure(
                MonotonicArbRejectReason::kINVALID_OBSERVATION);
        }

        const auto& top = market.asks[0];

        if (top.price_ticks == 0 ||
            top.price_ticks >= kPriceTicksPerDollar ||
            top.quantity_lots == 0) {
            return candidate_failure(
                MonotonicArbRejectReason::kINVALID_OBSERVATION);
        }
        ...
        MonotonicArbRejectReason candidate_stop_reason{
            MonotonicArbRejectReason::kNONE
        };
        ...
        for (std::size_t index = 0;
            index < market.ask_count;
            ++index) {

            const auto& level = market.asks[index];

            if (level.price_ticks == 0 ||
                level.price_ticks >= kPriceTicksPerDollar ||
            ...
            if (index > 0) {
                ...
                adjacent_gap =
                    level.price_ticks - previous_price;
            ...
            const PriceTicks distance_from_top =
                level.price_ticks - top.price_ticks;

            if (inside_near_top_window &&
                distance_from_top <=
                    config.near_top_depth_window_ticks) {
            ...
            const bool within_candidate_scan =
                index < config.maximum_easier_book_levels;

            if (within_candidate_scan && candidate_scan_active) {
                if (index > 0 &&
                    config.require_top_gap_continuity &&
                    adjacent_gap > config.maximum_top_gap_ticks) {

                    candidate_scan_active = false;
                    candidate_stop_reason =
                        MonotonicArbRejectReason::kDEPTH_DISCONTINUITY;
                }

                const PriceTicks aggression =
                    level.price_ticks - top.price_ticks;

                if (candidate_scan_active &&
                    ((!config.bounded_easier_aggression_enabled &&
                    aggression > 0) ||
                    aggression >
                        config.maximum_easier_aggression_ticks)) {

                    candidate_scan_active = false;
                    candidate_stop_reason =
                        MonotonicArbRejectReason::
                            kPRICE_AGGRESSION_EXCEEDED;
                }

                if (candidate_scan_active) {

                    candidate_quantity = add_up_to_required(
                    candidate_quantity,
                    level.quantity_lots,
                    ...
                    if (!config
                        ...
                        result.selections[result.count++] =
                            LegSelection{
                                .observed_top_ticks =
                                    top.price_ticks,
                                .chosen_limit_ticks =
                                ...
                                .scanned_levels =
                                    static_cast<std::uint8_t>(
                                        index + 1),
                                .cumulative_quantity_lots =
                                    candidate_quantity,
                                .bounded_aggression_applied =
                                    level.price_ticks !=
                                    top.price_ticks,
                            };
                    }
        ...
        if (config.require_top_gap_continuity &&
            (!observed_second_level || !top_gap_valid)) {
            return candidate_failure(
                MonotonicArbRejectReason::kDEPTH_DISCONTINUITY);
        }
        ...
        if (config.require_near_top_multilevel_support) {
            if (near_top_levels <
                config.minimum_near_top_levels) {
                return candidate_failure(
                    MonotonicArbRejectReason::kDEPTH_DISCONTINUITY);
            }

            if (near_top_quantity <
                config.order_quantity_lots) {
                return candidate_failure(
                    MonotonicArbRejectReason::kINSUFFICIENT_QUANTITY);
            }
        }
...
//NOLINTNEXTLINE(readability-function-cognitive-complexity) -- complex as all logic is inline for ease of reasoning around
    [[nodiscard]] LegCandidateSet collect_harder_candidates(
        const StrategyMarketView& market,
        const MonotonicArbConfig& config) noexcept {
                    if (market.bid_count == 0 ||
            market.bid_count > predex::strategy::kStrategyBookDepth) {
            return candidate_failure(
                MonotonicArbRejectReason::kINVALID_OBSERVATION);
        }
        ...
        for (std::size_t index = 0;
            ...
            const PriceTicks distance_from_top =
                top.price_ticks - level.price_ticks;
            ...
            const bool within_candidate_scan =
                index < config.maximum_harder_book_levels;
            ...
            if (within_candidate_scan && candidate_scan_active) {
                ...
                if (candidate_scan_active &&
                    ((!config.bounded_harder_aggression_enabled &&
                    aggression > 0) ||
                    aggression >
                        config.maximum_harder_aggression_ticks)) {
    ...
    [[nodiscard]] std::uint64_t calculate_gross_edge_ticks(
        PriceTicks easier_limit,
        ...
        const PriceTicks edge_per_contract =
            harder_limit - easier_limit;

        return (edge_per_contract * quantity_lots) /
            predex::strategy::kQuantityLotsPerContract;
    }

    inline constexpr QtyLots kMaximumSafeGrossQuantityLots =
        std::numeric_limits<std::uint64_t>::max() /
        (kPriceTicksPerDollar - 1);
    ...
    using predex::strategy::ArbAction;
        ...
        //NOLINTNEXTLINE
    [[nodiscard]] PairedSelectionResult select_paired_limits(
        const LegCandidateSet& easier_candidates,
        ...
        const std::uint64_t required_net_edge =
            config.minimum_net_edge_ticks +
            config.edge_cushion_ticks;

        for (std::size_t easier_index = 0;
            easier_index < easier_candidates.count;
            ++easier_index) {

            const auto& easier =
                easier_candidates.selections[easier_index];

            const MonotonicArbLeg easier_leg{
                .market_id = 0,
                .action = ArbAction::kBUY_YES,
                .limit_price_ticks =
                    easier.chosen_limit_ticks,
                .quantity_lots =
                    config.order_quantity_lots,
            };

            const FeeCalculation easier_fee =
                calculate_fee(config, easier_leg);
            ...
            for (std::size_t harder_index = 0;
                harder_index < harder_candidates.count;
                ++harder_index) {

                const auto& harder =
                    harder_candidates.selections[harder_index];

                const std::uint64_t gross_edge =
                    calculate_gross_edge_ticks(
                        easier.chosen_limit_ticks,
                        harder.chosen_limit_ticks,
                        config.order_quantity_lots);
                ...
                const FeeCalculation harder_fee =
                    calculate_fee(config, harder_leg);
                ...
                const std::uint16_t depth_score =
                    static_cast<std::uint16_t>(
                        easier.scanned_levels) +
                    static_cast<std::uint16_t>(
                        harder.scanned_levels);
                ...
                const bool is_better =
                    ...
                    (depth_score == best_depth_score &&
                    net_edge >
                        result.selection.net_edge_ticks);
        ...
        if (!saw_gross_edge) {
            result.failure =
                MonotonicArbRejectReason::kNO_GROSS_EDGE;
        } else if (!saw_fee_surviving_edge) {
            result.failure =
                MonotonicArbRejectReason::kFEES_ERASE_EDGE;
        } else {
            result.failure =
                MonotonicArbRejectReason::kINSUFFICIENT_NET_EDGE;
        }
...
namespace predex::strategy{
    ...
    bool valid_monotonic_arb_config(
        const MonotonicArbConfig& config) noexcept {
        ...
        if (config.require_top_gap_continuity &&
            config.maximum_top_gap_ticks == 0) {
            return false;
        }
        ...
        if (config.order_quantity_lots >
            kMaximumSafeGrossQuantityLots) {
            return false;
        }
        ...
        if (config.taker_fee_rate_numerator >
            config.taker_fee_rate_denominator) {
            return false;
        }
    ...
    FeeCalculation calculate_fee(
        ...
        if (config.taker_fee_rate_denominator == 0 ||
            candidate_leg.limit_price_ticks == 0 ||
            candidate_leg.limit_price_ticks >=
                kPriceTicksPerDollar) {
        ...
        const WideUInt price =
            candidate_leg.limit_price_ticks;

        const WideUInt complement =
            kPriceTicksPerDollar - price;

        const WideUInt numerator =
            static_cast<WideUInt>(
                config.taker_fee_rate_numerator) *
            candidate_leg.quantity_lots *
            price *
            complement;

        const WideUInt denominator =
            static_cast<WideUInt>(
                config.taker_fee_rate_denominator) *
            kQuantityLotsPerContract *
            kPriceTicksPerDollar;

        // Ceiling division.
        const WideUInt base_fee_ticks =
            (numerator + denominator - 1) /
            denominator;

        const WideUInt total_fee_ticks =
            base_fee_ticks +
            config.execution_rounding_reserve_ticks_per_leg;
        ...
        result.total_fee_ticks =
            static_cast<std::uint64_t>(total_fee_ticks);
    ...
    MonotonicArbEvaluation evaluate_monotonic_arb(
        const MonotonicArbConfig& config,
        ...
        if (!observation.easier.tradeable ||
            !observation.harder.tradeable) {
            return reject(
                MonotonicArbRejectReason::kMARKET_NOT_TRADEABLE);
        }

        const bool easier_closed =
            observation.easier.market_close_time_s != 0 &&
            wall_clock_now_s >=
                observation.easier.market_close_time_s;

        const bool harder_closed =
            observation.harder.market_close_time_s != 0 &&
            wall_clock_now_s >=
                observation.harder.market_close_time_s;

        if (easier_closed || harder_closed) {
            return reject(
                MonotonicArbRejectReason::kMARKET_CLOSED);
        }

        if (observation.easier.ask_count == 0) {
            return reject(
                MonotonicArbRejectReason::kMISSING_ASK_DEPTH);
        }

        if (observation.harder.bid_count == 0) {
            return reject(
                MonotonicArbRejectReason::kMISSING_BID_DEPTH);
        }
        const LegCandidateSet easier_candidates =
            collect_easier_candidates(
                observation.easier,
                config);
        ...
        return MonotonicArbEvaluation{
            ...
            .candidate = MonotonicArbCandidate{
                .event_id = observation.event_id,
                .event_revision = observation.event_revision,


                .easier_leg = MonotonicArbLeg{
                    .market_id =
                        observation.easier.market_id,
                    .action = ArbAction::kBUY_YES,
                    .limit_price_ticks =
                        selection.easier.chosen_limit_ticks,
                    .quantity_lots =
                        config.order_quantity_lots,
                },

                .harder_leg = MonotonicArbLeg{
                    .market_id =
                        observation.harder.market_id,
                    .action = ArbAction::kSELL_YES,
                    .limit_price_ticks =
                        selection.harder.chosen_limit_ticks,
                    .quantity_lots =
                        config.order_quantity_lots,
                },

                .gross_edge_ticks =
                    selection.gross_edge_ticks,
                .estimated_fee_ticks =
                    selection.estimated_fee_ticks,
                .net_edge_ticks =
                    selection.net_edge_ticks,
