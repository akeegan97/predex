...
- **Where**: `cpp/include/predex/shards/book_store.hpp:15` (`kMaxPriceTicks = 10000`), `:17-18` (BidLevels/AskLevels use `internal::QtyLots` = `std::int64_t`), `:25-26` (two full-range arrays per book), `:84` (`unordered_map<MarketId, BookState> books_`).
- **Impact**: Each side is `10001 × 8B ≈ 80KB`, each book `~160KB`. At 1000 markets that's `~160MB` of book state — 5–20× L3 depending on CPU; working set can't stay resident, so cross-market delta traffic eats DRAM latency (~100ns) for accesses that should be single-digit ns. The sizing was deliberate future-proofing for Kalshi's sub-penny pricing (0.01¢ granularity), but that tick class is currently limited to 15-min / up-down crypto markets which aren't in scope. Binary cents markets use >99% empty slots. Compounded by `std::unordered_map<MarketId, BookState>` scattering those 160KB blocks across the heap as separately-allocated nodes — no spatial locality between sibling markets on the same event.
- **Fix sketch**: Template the tick range: `BookState<TickRange>` and `BookStore<TickRange>`, default `<100>` for binary-cent markets. Route per shard by market tick-class (Kalshi's `get_markets` endpoint surfaces the flag; MarketRegistry can tag routes at startup). Sub-penny markets would land on a separate shard with `BookStore<10000>` specialization, introduced only when that tick class becomes in-scope. Result: ~800B/book × 1000 markets ≈ ~800KB total book memory, fits in L2 cluster-wide. Also switch `QtyLots` storage in book arrays to `std::uint32_t` (2× additional shrink; Kalshi per-level qty fits comfortably in 32 bits). Add cached `best_bid_tick` / `best_ask_tick` fields on BookState so ToB queries don't scan the dense array.
