...
class Logger {
  ...
  private:
    std::vector<predex::utils::SPSCQueue<predex::core::ingest::kalshi::FrameHandle>*>
        input_queues_; // terminal sink router/shard (eventually OMS queues as well) -> logger
    predex::core::ingest::kalshi::FramePool&
        frame_pool_; // shared frame pool for zero copy access to frames
    predex::utils::SPSCQueue<predex::core::ingest::kalshi::FrameHandle>&
        recycle_queue_;         // logger producer, IOWriter consumer
