...
void write_event_json(std::ofstream& output, const AuditEvent& event) {
    output << '{'
           // NOLINTNEXTLINE
           << "\"kind\":\"" << kind_to_string(event.kind) << "\"," << "\"ts_ns\":" << event.ts_ns
           << ',' << "\"shard_id\":" << event.shard_id << ',' << "\"signal_id\":" << event.signal_id
           << ',' << "\"group_id\":" << event.group_id << ','
           << "\"local_intent_id\":" << event.local_intent_id << ','
           << "\"oms_request_id\":" << event.oms_request_id << ','
           << "\"frame_seq\":" << event.frame_seq << ',' << "\"frame_sid\":" << event.frame_sid
           ...
           << "\"edge_ticks\":" << event.edge_ticks << ',' << "\"score\":" << event.score << ','
           ...
           << "\"market_exposure_lots\":"
           << event.market_exposure_lots
           /*
...
AuditLogger::AuditLogger(std::vector<predex::utils::SPSCQueue<AuditEvent>*> input_queues,
    ...
    if (!output_file_.is_open()) {
        throw std::runtime_error("Failed to open audit log file: " + std::string(output_file_path));
    }
}
