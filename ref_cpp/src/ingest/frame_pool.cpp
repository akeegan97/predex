...
FramePool::FramePool(std::size_t capacity) : capacity_(capacity) {
...
FramePool::~FramePool() { delete[] pool_; }
bool FramePool::try_acquire(FrameHandle& handle_out) noexcept {
    ...
    in_use_[idx] = 1;
    pool_[idx] = KalshiFrame{}; // zero out frame memory for safety, might want to optimize this
                                // later by only zeroing out relevant fields or relying on caller to
                                // zero out after acquiring
...
KalshiFrame* FramePool::writable_frame(const FrameHandle& handle) noexcept {
    ...
    if (generations_[handle.idx_] != handle.gen_) {
        assert(false &&
               "FramePool invariant violated: handle generation mismatch, possible use after free");
        return nullptr;
    }
...
bool FramePool::recycle(const FrameHandle& handle) noexcept {
    ...
    if (generations_[handle.idx_] != handle.gen_) {
        assert(false &&
               "FramePool invariant violated: handle generation mismatch, possible double free");
        return false;
    }
