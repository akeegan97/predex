...
MarketRegistry::MarketRegistry(const std::vector<MarketRegistryEntry>& entries) {
    ...
    for (const auto& entry : entries) {
        ...
        if (iter != routes_.end()) {
            // duplicate entry for ticker, could log this or handle differently if needed but for
            // now just skip/keep first entry
            continue;
        }
        routes_.emplace(entry.ticker_,
                        MarketRoute{entry.market_id_, entry.event_id_, entry.affinity_key_,
                                    entry.topology_kind_, entry.strike_key_});
    }
}
...
std::size_t MarketRegistry::size() const noexcept { return routes_.size(); }
} // namespace predex::core::routing::kalshi