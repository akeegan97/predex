...
ShardDispatch::ShardDispatch(std::vector<Queue*> shard_queues)
    : shard_queues_(std::move(shard_queues)) {}

bool ShardDispatch::try_dispatch(std::size_t shard_id,
                                 const predex::core::ingest::kalshi::FrameHandle& handle) noexcept {
...
ShardDispatchStats ShardDispatch::stats() const noexcept {
    return ShardDispatchStats{dispatched_count_.load(std::memory_order_relaxed),
                              dropped_count_.load(std::memory_order_relaxed)};
}

std::size_t ShardDispatch::shard_count() const noexcept { return shard_queues_.size(); }
} // namespace predex::core::routing::kalshi